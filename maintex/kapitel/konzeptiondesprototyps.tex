...
\begin{figure}[H]
    \centering
    \includegraphics[width=15cm]{images/Kapitel5/Moduldiagramm.png} 
...
Das System akzeptiert Gleispläne in PDF Format als primär Eingabe. PDF wurde als Standardformat gewählt, da es in der Praxis neben Bildformat am häufigsten für den Export technischer Zeichnungen verwendet wird. Funktionale Anforderungen an das Eingabemodul:
\begin{itemize}
    \item Hochladen von einzelner oder mehrerer PDF Dateien
...
\begin{enumerate}
    \item \textbf{Objekterkennung} - Ein trainiertes Deep Learning Modell der YOLO Familie analysiert die vorverarbeiteten Bildausschnitte und identifiziert relevante Symbole (Signale, Weichen, Balisen, etc.). Für jedes erkannte Objekt liefert das Modell:
    \begin{itemize}
...
\begin{enumerate}
    \item \textbf{Datenbereinigung:} Rohe OCR Ergebnisse enthalten häufig Artefakte, Formatierungsfehler oder unerwünschte Zeichen. Regelbasierte Filter (z.B. mittels parametrische Beziehungen) normalisieren die extrahierten Texte gemäß erwarteten Mustern.
...
\begin{itemize}
    ...
    \item \textbf{Nachverfolbarkeit:} Jedem Datensatz werden Metadaten zugeordnet(Quelldokument, Seitennummer, Koordinaten), um die Rückverfolgbarkeit zur Originalquelle zu gewährleisten.
...
\begin{itemize}
    ...
    \item \textbf{Fehlerbehandlung:} Klare Fehlermeldungen und Hilfstellungen bei Problemen
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{images/Kapitel5/Prototyp flowchar v01.png} 
...
\begin{enumerate}
    ...
    \item \textbf{Nachbearbeitung:} Zusammenführung der Tile Ergebnisse, Bereinigung und semantisches Mapping
    \item \textbf{Versionsspeicherung:} Persistierung der Ergebnisse mit Versionsmetadaten
    \item \textbf{Differenzanalyse:} Vergleich mit vorheriger Version bei wiederholter Analyse
    \item \textbf{Exportvorbereitung:} Transformation in Zielformat
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Die statischen Daten} umfassen die Basiskonfigurationsdaten (im YAML- und JSON-Format), die die Regeln für das Mapping definieren, sowie die trainierten Gewichte des YOLO-Modells. Diese Daten dienen als feste Eingabeparameter für die Verarbeitung im Backend. 
    \item \textbf{Die dynamischen Daten} werden während des Prozesses generiert. Zu diesem Zweck ist eine zentrale Datenbank (beispielsweise PostgreSQL) für die dauerhafte Speicherung der Ergebnisse erforderlich. Darüber hinaus werden temporäre Daten, wie die Anzeigebilder und Overlays, sowie die endgültigen Exportdaten in Formaten wie Excel (.xlsx), CSV oder JSON benötigt, die für den Nutzer bereitgestellt werden. 
\end{itemize}
...
Anti-Aliasing) und die Implementierung der Pipeline wird in Abschnitt 6.2.1 
...
bedingten Formatierung und Formelintegration, wird in Abschnitt 6.6.2 
...
Formulierung und der Algorithmen, wird in Abschnitt 6.6 detailliert beschrieben.
...
\begin{enumerate}
  ...
  nutzt diese zur Verbesserung zukünftiger Zuordnungen (vgl. Abschnitt 6.4.3).
\end{enumerate}
