...
\section{Prototyparchitektur}
Die Architektur des Prototyps folgt dem Prinzip der Modularität und Separation of Concerns.
Der Prototyp ist in funktional abgegrenzte Module unterteilt, die über definierte Schnittstellen miteinander kommunizieren. Diese Architektur ermöglicht eine schrittweise Entwicklung, einfache Wartbarkeit und die Möglichkeit zur späteren Erweiterung um zusätzliche Funktionalitäten \textbf{(Anforderungen \hyperref[req:FA-014]{FA-014}, \hyperref[req:NFA-008]{NFA-008})}. Abbildung \ref{fig:Systemarchitektur} zeigt die Gesamtarchitektur des Prototyps mit ihren Hauptkomponenten.

\begin{figure}[H]
    \centering
    \includegraphics[width=15cm]{images/Kapitel5/Moduldiagrammv3.png} 
    \caption{Modulare Prototyparchitektur des Gleisplanextraktors}
    \label{fig:Systemarchitektur}
\end{figure}

\subsection{Dateneingabe}

Der Prototyp akzeptiert Gleispläne in verschiedenen Formaten als primäre Eingabe 
\textbf{(Anforderung \hyperref[req:NFA-009]{NFA-009})}. Die Unterstützung mehrerer Formate berücksichtigt 
...
\begin{itemize}
    ...
    Der Prototyp rendert PDFs intern bei einer festen Auflösung von 500 DPI, was 
...
dass die Eingabe mit 500 DPI erstellt wurde. Der Prototyp zeigt einen entsprechenden
...
für eine ausreichende Auflösung beim Benutzer. Der Prototyp zeigt daher bei 
...
\begin{itemize}
    ...
    \item \textbf{Ergebnisvalidierung:} Visuelle Darstellung der erkannten Objekte überlagert auf dem originalen Gleisplan.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=16cm]{images/Kapitel5/Prototyp flowchar v03.png} 
...
Die Auswahl von \texttt{pdf2image} erfolgte nicht isoliert, sondern im direkten Vergleich mit etablierten Alternativen aus dem Python-Ökosystem. Da die PDF-Rasterisierung den ersten und damit kritischen Schritt der gesamten Verarbeitungspipeline darstellt, wurden die Kandidaten systematisch evaluiert. Rendering-Qualität, Performance und API-Design bildeten die technische Bewertungsgrundlage. Besonders relevant im industriellen Kontext erwies sich jedoch die lizenzrechtliche Kompatibilität. Dieser Aspekt ist für den Einsatz innerhalb von Siemens Mobility von zentraler Bedeutung, da bestimmte Open-Source-Lizenzen (insbesondere Copyleft-Lizenzen wie AGPL) die Verwendung in proprietären Systemen erheblich einschränken oder vollständig ausschließen können. Die folgende Tabelle fasst die identifizierten Ausschlusskriterien für die evaluierten Alternativen zusammen:
...
Anti-Aliasing) und die Implementierung der Pipeline wird in Abschnitt \ref{subsec:inferenz}
...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
Die Entscheidung für YOLOv8-OBB adressiert direkt die Anforderungen \textbf{\hyperref[req:FA-001]{FA-001}}
(Erkennungsrate $\geq$ 90\%), \textbf{\hyperref[req:FA-002]{FA-002}} (Rotationsinvarianz durch OBB-Regression)
und \textbf{\hyperref[req:NFA-007]{NFA-007}} (Ressourceneffizienz durch Echtzeitfähigkeit).
...
Der Prototyp kombiniert die Deep-Learning-basierte Bibliothek \textbf{PaddleOCR} (als Primärinstanz) mit der klassischen Engine \textbf{Tesseract} (als Fallback). Ergänzt wird dies durch einen eigens entwickelten \textbf{Dual-Angle-Routing-Algorithmus}.
...
Abbildung~\ref{fig:dual_angle_comparison} veranschaulicht dieses Konzept: Der Cardinal Path verwendet diskrete Rotation für nahezu ausgerichtete Texte ($|\theta| \leq 12^\circ$), während der Angular Path Perspektiventransformation für stark geneigte Texte ($|\theta| > 12^\circ$) anwendet.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \begin{scope}
        ...
        \node[below, font=\small] at (1.5, 2.8) {$|\theta| \leq 12^\circ$};
    ...
    \begin{scope}[xshift=6cm]
        ...
        \node[below, font=\small] at (1.5, 2.8) {$|\theta| > 12^\circ$};
    ...
    \end{scope}
\end{tikzpicture}
\caption{Vergleich der Dual-Winkel-Verarbeitungsstrategien}
...
Für Fälle, in denen PaddleOCR keine plausiblen Ergebnisse liefert (Konfidenz $< 0.5$), greift der Prototyp auf \textbf{Tesseract 5} zurück.
\begin{itemize}
...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
Die Multi-Engine-Strategie mit Dual-Angle-Routing erfüllt die Anforderungen \textbf{\hyperref[req:FA-004]{FA-004}}
(OCR-Genauigkeit) und \textbf{\hyperref[req:FA-005]{FA-005}} (OCR-Robustheit bei Rotation und Rauschen).
...
(Excel-Export-Modul) beschrieben. Diese Architekturentscheidung erfüllt die Anforderungen \textbf{\hyperref[req:FA-009]{FA-009}} (Excel-Integration),
\textbf{\hyperref[req:FA-010]{FA-010}} (Strukturerhalt bei bestehendem Template) und \textbf{\hyperref[req:NFA-010]{NFA-010}} (Ausgabeformate).
...
\begin{table}[H]
\centering
\caption{Alternativen zur hybriden Rückverfolgbarkeit}
...
Der Prototyp unterstützt die Versionierung von Gleisplänen, um Änderungen zwischen 
...
\begin{enumerate}
  ...
  \item \textbf{Differenzbildung}: Mittels Hungarian Algorithm werden Elemente optimal
        zugeordnet und Änderungen kategorisiert:
        \begin{itemize}
          ...
          \item \textbf{Verschoben}: Objekte mit geänderter Kilometrierung (coord\_value)
        \end{itemize}
\end{enumerate}

\subsubsection{Optimale Zuordnung mittels Hungarian Algorithm}

Für die Zuordnung von Elementen zwischen zwei Planversionen wird der Hungarian Algorithm
eingesetzt, der eine global optimale Zuweisung garantiert. Dies ist besonders wichtig bei:

\begin{itemize}
  \item \textbf{Duplikaten}: Mehrere gleichartige Elemente an ähnlichen Positionen
  \item \textbf{Fehlenden Bezeichnern}: Symbol-Klassen ohne eindeutige Textkennung
\end{itemize}
...
\begin{enumerate}
    ...
    \end{tabular}
    \end{table}
Die Wahl von PyQt5 als Desktop-Framework adressiert die Anforderungen \textbf{\hyperref[req:FA-013]{FA-013}}
(GUI ohne CLI-Kenntnisse), \textbf{\hyperref[req:NFA-001]{NFA-001}} (On-Premise-Verarbeitung ohne Cloud) und
\textbf{\hyperref[req:FA-012]{FA-012}} (visuelle Validierung durch Overlay-System).
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Binäre Topologie-Daten (BYTEA): Die extrahierte Gleistopologie (der Graph der Fahrstraßen) liegt zur Laufzeit als speicheroptimiertes NumPy-Array vor. Die Konvertierung in Textform (JSON/CSV) wäre ineffizient und würde Präzision kosten. Daher wird dieses Array direkt als binäres Objekt (BYTEA) persistiert, was Ladezeiten minimiert und die exakte Repräsentation der Gleisgeometrie wahrt.
    \end{itemize}
...
Das Datenbankdesign unterstützt die Anforderungen \textbf{\hyperref[req:NFA-005]{NFA-005}} (Prüfbarkeit durch
Metadaten-Persistierung), \textbf{\hyperref[req:FA-011]{FA-011}} (Änderungsverfolgung durch Versionierung) und \textbf{\hyperref[req:NFA-002]{NFA-002}} (Lizenzkonformität durch Open-Source-Datenbank).
...
zwischen erkannten Symbolen und den zugehörigen Texten (IDs, Koordinaten). 
Dieses Modul adressiert die Anforderungen \textbf{\hyperref[req:FA-006]{FA-006}} (Fahrtrichtungsdetektion) und
\textbf{\hyperref[req:FA-007]{FA-007}} (Symbol-Koordinaten-Verknüpfung).
...
\begin{enumerate}
  ...
  Der Prototyp lernt wiederkehrende Layoutmuster aus erfolgreichen Verknüpfungen und 
...
Textkandidaten werden innerhalb klassenspezifischer Suchradien um das Symbol gesucht. Die Auswahl erfolgt nach:
...
Das Validierungskonzept adressiert die Anforderungen \textbf{\hyperref[req:FA-008]{FA-008}} (manuelle Korrektur durch Human-in-the-Loop), \textbf{\hyperref[req:NFA-004]{NFA-004}} (Robustheit durch Fehlerbehandlung) und
\textbf{\hyperref[req:NFA-005]{NFA-005}} (Prüfbarkeit durch strukturierte Validierungsberichte). Das Validierungskonzept gliedert sich in drei Ebenen:
