...
\end{tabular}
\end{table}
\textbf{Lizenzkonformität (\hyperref[req:NFA-002]{NFA-002}):} Alle verwendeten Bibliotheken unterliegen 
...
\textbf{Modularität (\hyperref[req:FA-014]{FA-014}, \hyperref[req:NFA-008]{NFA-008}):} Die dargestellte Projektstruktur gewährleistet 
...
% =============================================================================
% COMPLETE SECTION 6.2 — Objekterkennung mit YOLOv8-OBB
% Replaces everything from \section{Objekterkennung mit YOLOv8-OBB}
% up to (but NOT including) the next \section (e.g., 6.3 OCR)
%
% STRUCTURE:
%   6.2   Objekterkennung mit YOLOv8-OBB (intro paragraph)
...
%         - Trainingskonvergenz
%         - Trainingsergebnisse
%   6.2.3 Inferenz-Pipeline  [NOT INCLUDED — keep your existing version or write separately]
%
% ALL NUMBERS VERIFIED AGAINST:
%   - Dataset stats (482 original, 1131 augmented, 15582 instances)
%   - results.csv (120 epochs)
%   - PR curve (mAP@0.5 = 0.984 all classes)
%   - F1 curve (0.96 at conf 0.674)
%   - Confusion matrices (absolute + normalized)
%
% BETREUER FEEDBACK ADDRESSED:
%   [1] 24 Gleispläne now stated
%   [2] 115 → 208 val images (old split removed)
%   [3] Augmentation purpose clarified (rotation robustness + class balance)
%   [4] Coordinate dominance explained
%   [5] S-Verbinder/Isolierstoß plausibility addressed
%   [6] All 13 classes evaluated (coordinate as linking class)
%   [7] Training section added with full hyperparameters + convergence (YOLOv8l-OBB)
% =============================================================================

\section{Objekterkennung mit YOLOv8-OBB}
\label{sec:yolo_implementation}

Die Objekterkennung bildet den ersten Schritt der Extraktionspipeline und ist für die zuverlässige Lokalisierung und Klassifikation aller relevanten Symbole in den Gleisplänen verantwortlich. Wie in den Grundlagen (Abschnitt~\ref{sec:orientedboundingboxesalslösung}) dargelegt, erfordert die rotationsvariierende Darstellung von Gleisplansymbolen den Einsatz von Oriented Bounding Boxes (OBB). Im Folgenden werden die Erstellung des Trainingsdatensatzes, die Durchführung und Optimierung des Modelltrainings sowie die daraus resultierenden Erkennungsleistungen beschrieben.
...
Der finale Datensatz umfasst insgesamt \textbf{13~Symbolklassen} gemäß Anforderung \hyperref[req:FA-003]{FA-003}: \textit{coordinate}, \textit{signal}, \textit{gks\_festkodiert}, \textit{gks\_gesteuert}, \textit{gm\_block}, \textit{sverbinder}, \textit{haltepunkt}, \textit{weichen\_block}, \textit{isolierstoß}, \textit{prellbock}, \textit{haltetafel}, \textit{weichenende} und \textit{weichengruppenende}. Die Klasse \textit{coordinate} nimmt eine besondere Rolle ein: Sie liefert die Positionsinformation (Kilometrierung), die den anderen 12~Klassen zugeordnet wird. Alle 13~Klassen werden in Kapitel~\ref{chap:evaluation} quantitativ evaluiert.
...
\begin{enumerate}
    \item \textbf{Klassenungleichgewicht:} Die natürliche Verteilung der Symbolklassen in den 482~annotierten Original-Tiles ist stark unausgeglichen. Häufige Klassen wie \textit{coordinate} dominieren, während seltene Klassen wie \textit{weichenende} oder \textit{weichengruppenende} nur in wenigen Tiles vorkommen. Ohne Gegenmaßnahmen besteht die Gefahr, dass das Modell auf häufige Klassen overfittet und seltene Symbole unzureichend lernt.
    
    \item \textbf{Orientierungsvielfalt:} Obwohl technische Zeichnungen Symbole in verschiedenen Winkeln enthalten, sind bestimmte Orientierungen -- insbesondere $0^\circ$ und $90^\circ$ -- in den realen Plänen überrepräsentiert, während diagonale Ausrichtungen seltener auftreten. Da das Modell in der Praxis Symbole unter beliebigen Rotationswinkeln zuverlässig erkennen muss, ist eine ausreichende Abdeckung aller relevanten Orientierungen im Trainingsdatensatz erforderlich.
\end{enumerate}

\paragraph{Augmentationsstrategie.}
Die synthetische Rotation wurde selektiv auf Bildausschnitte angewendet, die Instanzen unterrepräsentierter Klassen enthalten. Dabei werden keine künstlichen Symbole erzeugt, sondern bestehende, real annotierte Tiles mitsamt ihren OBB-Annotationen um definierte Winkel rotiert. Auf diese Weise trainiert das Modell dieselben Symbole in zusätzlichen Orientierungen. Für jede ausgewählte Tile wurden Rotationen um die folgenden Winkel durchgeführt:
...
entsprechend transformiert wurden. Jeder Eckpunkt $(x_i, y_i)$ einer OBB wird relativ zum Rotationszentrum transformiert, sodass die geometrische Korrektheit der Annotation erhalten bleibt. In Kombination mit der OBB-Architektur wird durch die 10~Rotationswinkel eine vollständige Rotationsinvarianz von $0^\circ$ bis $360^\circ$ sichergestellt (vgl.\ Anforderung \hyperref[req:FA-002]{FA-002}).

\paragraph{Klassenspezifische Steuerung.}
Die Anzahl der pro Tile generierten synthetischen Kopien wurde klassenabhängig gesteuert, um eine Zielverteilung von ca.\ 600~Instanzen pro unterrepräsentierter Klasse zu erreichen. Tiles, die ausschließlich bereits häufige Klassen wie \textit{coordinate} enthalten, wurden nur moderat augmentiert, um deren ohnehin hohe Ausgangszahl nicht unverhältnismäßig weiter zu steigern. Bei stark unterrepräsentierten Klassen (z.\,B.\ \textit{weichengruppenende} mit nur 74~Original-Instanzen oder \textit{haltetafel} mit 93~Original-Instanzen) wurden bis zu 10~Rotationsvarianten pro Tile generiert.

\paragraph{Technische Umsetzung.}
Die Implementierung nutzt \texttt{cv2.getRotationMatrix2D()} und \\ \texttt{cv2.warpAffine()} aus OpenCV. Die Bildgröße wurde dynamisch angepasst, um ein Abschneiden rotierter Objekte am Rand zu vermeiden. Die augmentierten Tiles wurden als separate Dateien mit eindeutiger Benennung (z.\,B.\ \texttt{*\_a+30.jpg} für eine $+30^\circ$-Rotation) im Datensatz gespeichert, was eine deterministische Reproduzierbarkeit gewährleistet. 

\paragraph{Validierung der Augmentation.}
Die transformierten OBB-Annotationen wurden stichprobenartig manuell auf
geometrische Korrektheit überprüft, um sicherzustellen, dass die
synthetische Rotation keine Artefakte einführt. Diese manuelle Validierung
ist primär bei der \textit{erstmaligen Implementierung} der
Augmentationspipeline erforderlich, um die korrekte Funktionsweise der
Rotationslogik und der OBB-Transformation zu verifizieren. Sobald die
Pipeline einmal validiert ist, sind die geometrischen Transformationen
deterministisch und mathematisch exakt, sodass bei der Hinzunahme neuer
Symbolklassen oder der Anwendung auf weitere Gleisplantypen keine erneute
manuelle Prüfung der Augmentation notwendig ist. Lediglich bei einer
Änderung der Augmentationslogik selbst, etwa der Einführung zusätzlicher
Transformationsarten wie Skalierung oder Spiegelung, wäre eine erneute
Validierung angezeigt.

\paragraph{Ergebnis.}
Durch die selektive Rotation wurden aus den 482~Original-Tiles zusätzlich \textbf{649~augmentierte Tiles} erzeugt, was den Datensatz auf insgesamt \textbf{1.131~Bildausschnitte} erweitert. Das Augmentationsverhältnis (57,4\,\% augmentiert) spiegelt den Fokus auf unterrepräsentierte Klassen wider: Tiles mit häufigen Symbolen wurden moderat oder gar nicht augmentiert, während Tiles mit seltenen Symbolen mehrfach rotiert wurden.

% ── 6.2.1.4  Datenaufteilung ────────────────────────────────────────────────
\subsubsection{Datenaufteilung und finale Statistiken}
\label{subsubsec:dataset_split}

\paragraph{Aufteilungsstrategie.}
Die Aufteilung des augmentierten Gesamtdatensatzes erfolgte stratifiziert mit einem \textbf{80/20-Verhältnis} in Trainings- und Validierungsdaten. Die Stratifizierung stellt sicher, dass die relative Klassenverteilung in beiden Teilmengen annähernd gleich bleibt. Tabelle~\ref{tab:dataset_split} zeigt die resultierende Aufteilung:

\begin{table}[H]
\centering
\caption{Aufteilung des Datensatzes in Trainings- und Validierungssatz}
\label{tab:dataset_split}
\begin{tabular}{|l|r|r|r|r|}
\hline
\textbf{Teilmenge} & \textbf{Original} & \textbf{Augmentiert} & \textbf{Gesamt Bilder} & \textbf{Instanzen} \\
\hline
Training     & 399 & 524 & 923 & 12.506 \\
Validierung  &  83 & 125 & 208 &  3.076 \\
\hline
\textbf{Gesamt} & \textbf{482} & \textbf{649} & \textbf{1.131} & \textbf{15.582} \\
\hline
\end{tabular}
\end{table}

Der Anteil augmentierter Bilder beträgt 56,8\,\% im Trainings- und 60,1\,\% im Validierungssatz. Die annähernd proportionale Klassenverteilung in Tabelle~\ref{tab:dataset_distribution} bestätigt die stratifizierte Aufteilung.

\paragraph{Klassenverteilung.}
Tabelle~\ref{tab:dataset_distribution} zeigt die vollständige Verteilung der Annotationen über alle 13~Symbolklassen, aufgeschlüsselt nach Trainings- und Validierungssatz. Die Spalte \textit{Bilder} gibt an, in wie vielen Bildern der jeweiligen Teilmenge mindestens eine Instanz der betreffenden Klasse vorkommt. Da ein einzelnes Bild typischerweise mehrere Symbolklassen gleichzeitig enthält, überlappen sich die Einzelwerte der Spalte \textit{Bilder} und summieren sich nicht zur Gesamtbildzahl. Die Klasse \textit{coordinate} dominiert mit 7.428~Instanzen (47,7\,\% aller Annotationen), da jedes Betriebselement im Gleisplan eine zugehörige Kilometrierungsangabe besitzt -- Signale, Weichen, GKS und Haltepunkte werden jeweils durch mindestens eine Koordinate referenziert. Gefolgt wird sie von \textit{signal} mit 2.348~Instanzen. Am seltensten vertreten sind \textit{weichengruppenende} (92~Instanzen) und \textit{haltetafel} (116~Instanzen), was die in Abschnitt~\ref{subsubsec:augmentation} beschriebene gezielte Augmentierung dieser Klassen motivierte.
\begin{table}[H]
\centering
\caption{Verteilung der Annotationen über alle 13~Klassen im Trainings- und Validierungssatz}
\label{tab:dataset_distribution}
\footnotesize
\begin{tabular}{|l|r|r|r|r|r|}
\hline
\textbf{Klasse} & \textbf{Train Bild.} & \textbf{Train Inst.} & \textbf{Val Bild.} & \textbf{Val Inst.} & \textbf{Gesamt} \\
\hline
coordinate       & 890 & 5.945 & 207 & 1.483 & 7.428 \\
signal           & 681 & 1.875 & 164 &   473 & 2.348 \\
gm\_block        & 585 &   843 & 143 &   201 & 1.044 \\
gks\_gesteuert   & 586 &   651 & 147 &   157 &   808 \\
gks\_festkodiert & 390 &   479 &  88 &   122 &   601 \\
sverbinder       & 611 &   703 & 140 &   150 &   853 \\
isolierstoß      & 264 &   618 &  67 &   147 &   765 \\
weichen\_block   & 325 &   514 &  67 &   129 &   643 \\
haltepunkt       & 376 &   388 &  92 &    92 &   480 \\
weichenende      & 170 &   198 &  50 &    50 &   248 \\
prellbock        & 109 &   125 &  31 &    31 &   156 \\
haltetafel       &  81 &    93 &  23 &    23 &   116 \\
weichengruppenende&  74 &    74 &  18 &    18 &    92 \\
\hline
\textbf{Gesamt} & \textbf{923\textsuperscript{*}} & \textbf{12.506} & \textbf{208\textsuperscript{*}} & \textbf{3.076} & \textbf{15.582} \\
\hline
\end{tabular}
\end{table}

Die Daten wurden in eine standardisierte YOLO-Verzeichnisstruktur (Abbildung~\ref{fig:COCODatenset}) überführt, welche die Organisation von Trainings- und Validierungsdaten sowie die zugehörigen Annotationen in separaten Verzeichnissen vorsieht. Diese Struktur trennt Bilder und Labels in dedizierte Unterverzeichnisse (\texttt{images/} und \texttt{labels/}), wobei jeweils eine weitere Unterteilung in \texttt{train/} und \texttt{val/} erfolgt. Jede Bilddatei besitzt eine korrespondierende Textdatei im Labels-Verzeichnis, die die OBB-Annotationen im YOLO-Format enthält (Klassen-ID, normalisierte Polygon-Koordinaten). Diese standardisierte Struktur ermöglicht die direkte Verwendung des Datensatzes mit der Ultralytics-Trainings-API ohne zusätzliche Konvertierungsschritte.
\begin{figure}[H]
    \centering
    \includegraphics[width=5cm]{images/Kapitel6/cocodatasetconfig.png} 
    \caption{YOLO-konforme Datensetstruktur für das YOLOv8-OBB-Training}
    \label{fig:COCODatenset}
\end{figure}

Abbildung~\ref{fig:labels_distribution} visualisiert die Charakteristika des Trainingsdatensatzes mit Klassenverteilung (oben links), räumlicher Verteilung der Bounding-Box-Zentren (unten links) und Größenverteilung der annotierten Objekte (unten rechts).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/Kapitel6/labels.jpg}
    \caption{Charakteristika des Trainingsdatensatzes (923~Bilder, 12.506~Instanzen)}
    \label{fig:labels_distribution}
\end{figure}

Die Abbildung zeigt die nach Augmentation resultierende Verteilung. Die Klassenverteilung zeigt die nach Augmentation resultierende Verteilung mit der erwartungsgemäß dominierenden Koordinaten-Klasse (5.945~Trainingsinstanzen), gefolgt von Signalen (1.875~Trainingsinstanzen). Die räumliche Verteilung der Bounding-Box-Zentren zeigt eine gleichmäßige Abdeckung des Bildbereichs ohne systematische Häufungen oder Leerräume, was auf eine gute Repräsentativität des Datensatzes hindeutet. Die Größenverteilung verdeutlicht die charakteristisch kleinen Objektdimensionen der Gleisplansymbole, die überwiegend im Bereich von 2--10\,\% der normalisierten Bilddimensionen liegen.

\paragraph{Diskussion der Klassenverteilung.}
Die \textbf{Dominanz der Koordinaten-Klasse} ist ein inhärentes Merkmal von Gleisplänen und kein Artefakt des Datensatzes: Grundsätzlich besitzt jedes Objekt in einem Gleisplan -- ob Signal, GKS, Weiche oder Haltepunkt -- eine eigene Kilometerangabe. Die Anzahl der Koordinatenbeschriftungen entspricht daher annähernd der Summe aller übrigen Objektinstanzen. Ein direkter Größenvergleich zwischen der Koordinaten-Klasse und einzelnen Symbolklassen ist somit methodisch nur bedingt aussagekräftig, da Koordinaten funktional eine Referenzklasse darstellen, die alle anderen Symbolklassen begleitet.

Die \textbf{Instanzzahlen einzelner Klassen} variieren entsprechend ihrer natürlichen Häufigkeit in Gleisplänen. Aus eisenbahntechnischer Sicht wäre zu erwarten, dass S-Verbinder und Isolierstöße in realen Gleisplänen mindestens ebenso häufig vorkommen wie Signale, da jedes Signal theoretisch einen S-Verbinder oder Isolierstoß in unmittelbarer Nähe besitzt, ergänzt durch weitere Vorkommen am Ende des Durchrutschweges, in Weichenbereichen und als Trennung mehrerer Gleisabschnitte \cite{Pachl}. Die im Datensatz beobachteten Instanzzahlen spiegeln die tatsächliche Verteilung in den annotierten 24~Gleisplänen wider. Die detaillierte quantitative Evaluation der Erkennungsleistung über alle 13~Klassen erfolgt in Kapitel~\ref{chap:evaluation}.
...
% ── 6.2.2.1  Architektur und Hyperparameter ──────────────────────────────────
\subsubsection{Modellarchitektur und Hyperparameter}

Als Modellarchitektur wurde \textbf{YOLOv8l-OBB} gewählt, die \textit{Large}-Variante der YOLOv8-Familie mit OBB-Erweiterung. Diese Variante bietet mit ca.\ 43,7~Millionen Parametern eine hohe Modellkapazität, die für die zuverlässige Unterscheidung der 13~visuell teilweise ähnlichen Symbolklassen -- insbesondere der GKS-Varianten -- erforderlich ist. Im Vergleich zur kleineren \textit{Small}-Variante (11,2~Mio.\ Parameter) erreicht die \textit{Large}-Variante eine höhere Erkennungsgenauigkeit bei feingranularen Klassenunterscheidungen, wobei die Inferenzgeschwindigkeit für den Batch-Verarbeitungsmodus der Gleisplanpipeline weiterhin ausreichend ist.

Das Training wurde auf einer cloud-basierten GPU-Instanz (NVIDIA, 16~GB
Videospeicher) durchgeführt, da lokale Entwicklungsmaschinen nicht über
ausreichende Rechenleistung verfügen, um das Training in angemessener Zeit
abzuschließen. Diese Cloud-Ressource ist jedoch \textit{ausschließlich für
die initiale Trainingsphase} erforderlich. Das resultierende trainierte
Modell (\texttt{best.pt}, ca.\ 87~MB) kann anschließend auf
Standard-Hardware ohne GPU für die Inferenz eingesetzt werden
(vgl.\ Abschnitt~\ref{subsec:inferenz}). Für zukünftige Projekte ergeben
sich zwei Szenarien: Wenn ein bestehendes Modell bereits die benötigten
Symbolklassen abdeckt, ist \textit{kein erneutes Training} notwendig --
das Modell wird direkt wiederverwendet. Nur bei der Erweiterung um neue,
bisher nicht trainierte Symbolklassen oder bei der Anpassung an einen
grundlegend anderen Gleisplantyp ist ein erneutes Training auf einer
GPU-Instanz erforderlich. Durch den Einsatz von Transfer Learning
(vgl.\ Tabelle~\ref{tab:training_hyperparameters}) kann dabei auf den
bereits trainierten Gewichten aufgebaut werden, was die benötigte
Trainingszeit und Datenmenge für solche Erweiterungen deutlich reduziert.

Das Modell wurde mit auf dem DOTA-Datensatz \cite{Xia2018_DOTA}
vortrainierten Gewichten initialisiert (\textit{Transfer Learning}), um die
in den frühen Netzwerkschichten bereits gelernten generischen
Merkmalsextraktoren -- wie Kanten-, Ecken- und Texturerkennung -- zu nutzen
und die Konvergenz auf dem domänenspezifischen Gleisplan-Datensatz zu
beschleunigen.

Tabelle~\ref{tab:training_hyperparameters} fasst die verwendeten Hyperparameter zusammen:

\begin{table}[H]
\centering
\caption{Hyperparameter des YOLOv8l-OBB-Trainings}
...
Vortraining & DOTA-Weights & Transfer Learning \\
Eingabegröße & $1024 \times 1024$~px & Angepasst an Tile-Dimensionen \\
Epochen & 120 & Konvergenz ab Epoche~$\sim$60 \\
Batch-Größe & 8 & Limitiert durch 16~GB VRAM \\
Optimizer & SGD & YOLOv8-Standard \\
Initiale Lernrate & 0,01 & YOLOv8-Default \\
Finale Lernrate & 0,0002 & Cosine Annealing ($\text{lrf}=0{,}01$) \\
...
Die \textbf{Eingabegröße} von $1024 \times 1024$~Pixel weicht vom YOLOv8-Standard (640~Pixel) ab und wurde bewusst gewählt, um der Kleinteiligkeit der Gleisplansymbole gerecht zu werden. Bei einer Standard-Eingabegröße von 640~Pixel würden die bereits kleinen Symbole (vgl.\ Größenverteilung in Abbildung~\ref{fig:labels_distribution}) weiter verkleinert und könnten unterhalb der effektiven Erkennungsgrenze des Feature-Extractors fallen.

Die \textbf{Batch-Größe} von 8 wurde durch den verfügbaren GPU-Speicher (16~GB VRAM) begrenzt. Die \textit{Large}-Variante des YOLOv8-OBB-Modells belegt mit 43,7~Millionen Parametern bereits einen erheblichen Anteil des GPU-Speichers. In Kombination mit der Eingabegröße von $1024 \times 1024$~Pixeln, den zugehörigen Aktivierungen und Gradienten ist eine Batch-Größe von 8 das Maximum innerhalb des verfügbaren Speicherbudgets.

Das \textbf{Lernraten-Scheduling} folgt dem in YOLOv8 implementierten \textit{Cosine Annealing} mit linearem Warmup. In den ersten drei Epochen wird die Lernrate linear von einem niedrigen Wert auf die initiale Lernrate von 0,01 erhöht, um instabile Gradienten in der Frühphase des Trainings zu vermeiden. Anschließend fällt die Lernrate gemäß einer Kosinus-Funktion kontinuierlich auf den finalen Wert von 0,0002 (1\,\% der Initialrate) ab. Dieses Schema ermöglicht in der mittleren Trainingsphase eine effiziente Exploration des Parameterraums, während in der Spätphase eine feinere Justierung der Gewichte durch die reduzierte Schrittweite erfolgt.

% ── 6.2.2.2  Konvergenz ─────────────────────────────────────────────────────
\subsubsection{Trainingskonvergenz}
\label{subsubsec:trainingskonvergenz}

Die Gesamtdauer des Trainings über 120~Epochen betrug \textbf{ca.\ 6~Stunden}. YOLOv8-OBB optimiert drei Verlustkomponenten simultan:

\begin{itemize}
    \item \textbf{Box Loss} (\textit{train/box\_loss}): Quantifiziert die Abweichung zwischen vorhergesagten und tatsächlichen Bounding-Box-Positionen und -Dimensionen.
    \item \textbf{Classification Loss} (\textit{train/cls\_loss}): Misst den Fehler bei der Klassenzuordnung der erkannten Objekte.
    \item \textbf{Distribution Focal Loss} (\textit{train/dfl\_loss}): Optimiert die Vorhersageverteilung der Bounding-Box-Eckpunkte und verbessert insbesondere die Lokalisierungsgenauigkeit bei kleinen Objekten.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/Kapitel6/results.png}
    \caption{Trainings- und Validierungsverläufe über 120~Epochen: 
    (oben)~Trainings-Losses und Metriken, (unten)~Validierungs-Losses und mAP-Werte.}
    \label{fig:training_curves}
\end{figure}

Abbildung~\ref{fig:training_curves} zeigt den Verlauf aller drei Verlustkomponenten sowie der Evaluationsmetriken über die 120~Trainingsepoch\-en. Der Trainingsverlauf lässt sich in drei charakteristische Phasen unterteilen:

\begin{enumerate}
    \item \textbf{Warmup-Phase (Epochen 1--3):} Die Lernrate wird linear auf den Initialwert hochgefahren. Alle Verlustkomponenten fallen steil ab, was die schnelle Adaption der vortrainierten DOTA-Gewichte an die Gleisplan-Domäne zeigt. Bereits nach Epoche~3 erreicht das Modell eine mAP@0,5 von 92,5\,\%.
    
    \item \textbf{Primäre Lernphase (Epochen 4--60):} Die Verluste sinken stetig, während Precision, Recall und mAP kontinuierlich ansteigen. Die mAP@0,5 steigt von 92,5\,\% auf ca.\ 98,0\,\%, die mAP@0,5:0,95 von 79,1\,\% auf ca.\ 92,0\,\%. In dieser Phase lernt das Modell die domänenspezifischen Merkmale der 13~Symbolklassen.
    
    \item \textbf{Feintuning-Phase (Epochen 61--120):} Die Validierungsmetriken stabilisieren sich auf hohem Niveau mit geringfügigen Schwankungen. Die Trainingsverluste sinken weiter -- insbesondere zeigt sich bei Epoche~$\sim$110 ein markanter Abfall (\textit{box\_loss}: 0,26$\to$0,21; \textit{cls\_loss}: 0,21$\to$0,17), der auf die finale Phase des Cosine-Annealing-Schedules zurückzuführen ist, in der die stark reduzierte Lernrate eine feinere Gewichtsoptimierung ermöglicht.
\end{enumerate}

\paragraph{Overfitting-Analyse.}
Ein entscheidendes Qualitätskriterium des Trainings ist die Abwesenheit von Overfitting, d.\,h.\ das Modell generalisiert auf ungesehene Daten (Validierungssatz) ähnlich gut wie auf Trainingsdaten. Die Validierungsverluste zeigen ab Epoche~50 einen stabilen Plateauverlauf ohne Anstieg: \textit{val/box\_loss} stabilisiert sich bei ca.\ 0,38, \textit{val/cls\_loss} bei ca.\ 0,28 und \textit{val/dfl\_loss} bei ca.\ 1,50. Da ein ansteigender Validierungsverlust bei gleichzeitig sinkendem Trainingsverlust das klassische Indiz für Overfitting wäre, kann dieses Phänomen für das vorliegende Training ausgeschlossen werden. Der moderate und stabile Gap zwischen Trainings- und Validierungsverlust (z.\,B.\ \textit{box\_loss}: 0,21 vs.\ 0,38) ist für augmentierte Datensätze typisch und reflektiert die zusätzliche Variabilität der Trainingsdaten durch die synthetische Rotation.

% ── 6.2.2.3  Ergebnisse ─────────────────────────────────────────────────────
\subsubsection{Trainingsergebnisse}
\label{subsubsec:trainingsergebnisse}

\paragraph{Aggregierte Metriken.}
Tabelle~\ref{tab:training_results} fasst die finalen Detektionsmetriken des trainierten Modells zusammen, evaluiert auf dem Validierungsdatensatz (208~Bilder, 3.076~Instanzen):

\begin{table}[H]
\centering
\caption{Aggregierte Detektionsmetriken auf dem Validierungsdatensatz (208~Bilder, instanzgewichteter Durchschnitt)}
...
Die berichteten Metriken beziehen sich auf das automatisch gesicherte Modell mit den besten Validierungsergebnissen (Epoche~103 von~120), welches als \texttt{best.pt} exportiert und für die produktive Inferenz verwendet wird. Die mAP@0,5 von \textbf{98,4\,\%} zeigt, dass das Modell bei einem IoU-Schwellenwert von 50\,\% nahezu alle Symbole korrekt lokalisiert und klassifiziert. Die strengere Metrik mAP@0,5:0,95, die über IoU-Schwellenwerte von 50\,\% bis 95\,\% mittelt, erreicht \textbf{92,7\,\%} und bestätigt eine hohe Lokalisierungspräzision auch bei strengen Überlappungsanforderungen. Der optimale F1-Score von 0,96 wird bei einer Konfidenzschwelle von 0,674 erreicht, was darauf hindeutet, dass das Modell überwiegend hochkonfidente Vorhersagen trifft.

\paragraph{Klassenspezifische Leistung.}
Abbildung~\ref{fig:pr_curve} zeigt die Precision-Recall-Kurven für alle 13~Symbolklassen, wobei die Werte in der Legende die jeweilige AP@0,5 angeben. Die klassenspezifischen AP@0,5-Werte sind in Tabelle~\ref{tab:per_class_ap} zusammengestellt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/Kapitel6/BoxPR_curve.png}
    \caption{Precision-Recall-Kurven für alle 13~Symbolklassen auf dem Validierungsdatensatz}
    \label{fig:pr_curve}
\end{figure}

\begin{table}[H]
\centering
\caption{Klassenspezifische AP@0,5 auf dem Validierungsdatensatz, sortiert nach absteigender AP@0,5}
\label{tab:per_class_ap}
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Klasse} & \textbf{Val Inst.} & \textbf{AP@0,5} & \textbf{Bewertung} \\
\hline
gm\_block        &   201 & 99,5\,\% & Exzellent \\
sverbinder       &   150 & 99,5\,\% & Exzellent \\
prellbock        &    31 & 99,5\,\% & Exzellent \\
weichenende      &    50 & 99,5\,\% & Exzellent \\
weichengruppenende&    18 & 99,0\,\% & Exzellent \\
coordinate       & 1.483 & 98,7\,\% & Exzellent \\
haltepunkt       &    92 & 98,7\,\% & Exzellent \\
haltetafel       &    23 & 98,4\,\% & Exzellent \\
gks\_festkodiert &   122 & 97,8\,\% & Sehr gut \\
isolierstoß      &   147 & 97,8\,\% & Sehr gut \\
weichen\_block   &   129 & 97,7\,\% & Sehr gut \\
signal           &   473 & 97,6\,\% & Sehr gut \\
gks\_gesteuert   &   157 & 95,1\,\% & Gut \\
\hline
\textbf{$\varnothing$ Alle Klassen} & \textbf{3.076} & \textbf{98,4\,\%} & \\
\hline
\end{tabular}
\end{table}

Bemerkenswert ist, dass \textbf{alle 13~Klassen eine AP@0,5 über 95\,\%} erreichen und die Streuung mit 95,1\,\% bis 99,5\,\% sehr gering ist. Dies bestätigt, dass die in Abschnitt~\ref{subsubsec:augmentation} beschriebene Augmentationsstrategie das Klassenungleichgewicht erfolgreich adressiert hat: Auch Klassen mit wenigen Validierungsinstanzen (z.\,B.\ \textit{weichengruppenende} mit nur 18~Instanzen) erreichen eine AP@0,5 von 99,0\,\%.

\paragraph{Fehleranalyse.}
Abbildung~\ref{fig:confusion_matrix} zeigt die normalisierte Konfusionsmatrix auf dem Validierungsdatensatz, wobei Zeilen die vorhergesagten Klassen und Spalten die tatsächlichen Klassen repräsentieren. Die Diagonale zeigt durchgehend hohe Werte ($\geq 0{,}93$), was eine zuverlässige Klassifikation über alle Klassen bestätigt. Die vereinzelten Fehlklassifikationen betreffen primär die visuell ähnlichen GKS-Varianten sowie kleine Symbole wie Isolierstöße an Tile-Grenzen. Eine detaillierte klassenspezifische Analyse der Detektionsleistung erfolgt in Kapitel~\ref{chap:evaluation}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/Kapitel6/confusion_matrix_normalized.png}
    \caption{Normalisierte Konfusionsmatrix auf dem Validierungsdatensatz}
    \label{fig:confusion_matrix}
\end{figure}
\newpage
\paragraph{Konfidenzkalibrierung.}
Abbildung~\ref{fig:f1_curve} zeigt die F1-Score-Konfidenz-Kurve für alle 13~Klassen, wobei das globale Optimum bei $F1 = 0{,}96$ bei einer Schwelle von 0,674 liegt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/Kapitel6/BoxF1_curve.png}
    \caption{F1-Score in Abhängigkeit der Konfidenzschwelle}
    \label{fig:f1_curve}
\end{figure}

Die Kurve zeigt, dass die Mehrzahl der Klassen über einen breiten Konfidenzbereich (0,1--0,9) einen F1-Score über 0,95 aufrechterhält, was auf eine gute Kalibrierung des Modells hindeutet. Der optimale globale F1-Score von 0,96 bei einer Konfidenzschwelle von 0,674 wird als Standard-Detektionsschwelle für die nachfolgende Inferenz-Pipeline übernommen. Eine detaillierte Evaluation der End-to-End-Leistung einschließlich der nachgelagerten OCR- und Verknüpfungsschritte erfolgt in Kapitel~\ref{chap:evaluation}.

Das trainierte Modell (\texttt{best.pt}) wurde als Gewichtsdatei exportiert und in die Inferenz-Pipeline des Prototyps integriert. Die im folgenden Abschnitt beschriebene Inferenz-Pipeline nutzt dieses Modell zur Verarbeitung neuer, bisher ungesehener Gleispläne.

\subsection{Inferenz-Pipeline Implementierung}
\label{subsec:inferenz}
...
(Anforderung \hyperref[req:NFA-009]{NFA-009}).
...
A0-Gleisplan resultiert bei 500 DPI in einem Bild von ca. $30000 \times 5000$ bis $90000 \times 7000$ Pixeln.
...
\textbf{Downsampling:} Die extrahierten Tiles mit einer Größe von $2048 \times 2048$ Pixeln werden vor der YOLO-Inferenz auf die Trainingsgröße von $1024 \times 1024$ Pixeln herunterskaliert. Downsampling (auch Unterabtastung oder Dezimation genannt) bezeichnet in der digitalen Bildverarbeitung die Reduktion der räumlichen Auflösung eines Bildes durch Verringerung der Pixelanzahl \cite{Gonzalez}. Bei einem Skalierungsfaktor von 2 wird jedes $2 \times 2$ Pixelfeld des Originalbildes zu einem einzelnen Pixel im Zielbild zusammengefasst, wobei typischerweise Interpolationsverfahren wie bilineare oder bikubische Interpolation zum Einsatz kommen, um Aliasing-Artefakte zu minimieren.
...
\text{Horizontal}, & \text{falls } |\theta_{\text{norm}}| < 12^\circ \text{ oder } |\theta_{\text{norm}} - 180^\circ| < 12^\circ \\
\text{Vertikal}, & \text{falls } |\theta_{\text{norm}} - 90^\circ| < 12^\circ \text{ oder } |\theta_{\text{norm}} - 270^\circ| < 12^\circ \\
...
und damit die Anforderungen \hyperref[req:FA-004]{FA-004} (OCR-Genauigkeit) und \hyperref[req:FA-005]{FA-005}
...
Der Ablauf dieser Multi-Engine-Kaskadierung ist in Abbildung~\ref{fig:ocr_cascade} dargestellt.
...
Anschließend wird geprüft, ob der Absolutwert des normalisierten Winkels unter $12^\circ$ liegt. Diese $12^\circ$ wurden empirisch durch Experimente bestimmt: Bei Winkeln bis zu dieser Größe liefert eine einfache Rotation auf den nächsten kardinalen Winkel ($0^\circ, 90^\circ, 180^\circ$ oder $270^\circ$) bessere Ergebnisse als eine komplexe Perspektiventransformation. Der Grund ist, dass bei kleinen Winkeln die Perspektiventransformation zu leichten Verzerrungen führen kann, während die diskrete Rotation verlustfrei ist.

\textbf{Cardinal Path (Winkel $\leq$ $12^\circ$):} Die vier kardinalen Kandidaten ($0^\circ, 90^\circ, 180^\circ, 270^\circ$) sind jeweils $90^\circ$ voneinander entfernt. Der Schwellenwert von $12^\circ$ bedeutet, dass der gemessene Winkel maximal $12^\circ$ vom nächstgelegenen Kardinalwinkel abweicht. Da die Kardinale $90^\circ$ auseinander liegen, deckt dieser Schwellenwert einen Bereich von $\pm12^\circ$ um jeden Kardinalwinkel ab -- also insgesamt $24^\circ$ von den $90^\circ$ zwischen zwei benachbarten Kardinalen. Bei Winkeln innerhalb dieses Bereichs wird der Bildausschnitt einfach auf den nächstgelegenen kardinalen Winkel rotiert. Die Rotation erfolgt mittels einer Standard-Rotationsmatrix, die das Bild um den Mittelpunkt dreht. Diese Methode ist schnell, einfach und vermeidet Interpolationsartefakte, da nur $90^\circ$-Schritte verwendet werden.

Konkret wird aus den vier Kandidaten derjenige gewählt, der die kleinste Winkeldifferenz zum normalisierten Winkel hat. Ein Winkel von $10^\circ$ liegt innerhalb des $12^\circ$-Schwellenwerts und wird daher auf $0^\circ$ rotiert; ein Winkel von $85^\circ$ wird auf $90^\circ$ rotiert, da die Differenz nur $5^\circ$ beträgt.


\textbf{Angular Path (Winkel > $12^\circ$):} Bei größeren Abweichungen von den kardinalen Winkeln wird eine Perspektiventransformation verwendet. Diese Methode nutzt die vier Eckpunkte der Oriented Bounding Box (OBB), um eine Transformationsmatrix zu berechnen. Die Transformation wandelt die gedrehte OBB in ein achsenparalleles Rechteck um, wodurch der Text horizontal ausgerichtet wird.
...
Obwohl diese Methode rechenintensiver ist (etwa 2-3 Mal langsamer als eine einfache Rotation), liefert sie bei beliebigen Winkeln deutlich bessere Ergebnisse, da der Text exakt horizontal ausgerichtet wird, nicht nur annähernd. Abbildung~\ref{fig:dual_path} veranschaulicht den Unterschied zwischen beiden Rotationsstrategien.
...
\begin{scope}[yshift=0cm] % Lokaler Bereich oben
    \node at (0, 1.5) {\textbf{Cardinal Path ($|\theta| \leq 12^\circ$)}};
...
\begin{scope}[yshift=-4cm] 
    \node at (0, 1.5) {\textbf{Angular Path ($|\theta| > 12^\circ$)}};
...
Das System erkannte den Koordinatentext \texttt{0.0629} korrekt trotz der nicht-kardinalen Orientierung von $\theta = -37{,}5^\circ$. Die Perspektiventransformation richtete die gedrehte Oriented Bounding Box horizontal aus, wodurch PaddleOCR mit einer Konfidenz von 0.95 den Text extrahieren konnte. Die erfolgreiche Verknüpfung mit dem benachbarten Symbol (Gl.112) validiert sowohl die OCR-Robustheit (\hyperref[req:FA-005]{FA-005}) als auch die Rotationsinvarianz (\hyperref[req:FA-002]{FA-002}) für beliebige Winkel außerhalb der kardinalen Orientierungen.
...
Vor der eigentlichen OCR-Verarbeitung werden die Bildausschnitte durch eine Reihe von Vorverarbeitungsschritten optimiert, um die Textqualität zu verbessern und störende Elemente zu entfernen. Abbildung~\ref{fig:preprocessing} zeigt diese Schritte schematisch.
...
Der Parameter \enquote{Clip Limit} (hier: 2.0) begrenzt die maximale Verstärkung des Kontrasts. Ohne diese Begrenzung würde CLAHE in homogenen Bereichen (wie einem gleichmäßigen Hintergrund) minimales Rauschen extrem verstärken. Der Wert 2.0 bedeutet, dass die Histogrammverstärkung auf das Doppelte des Durchschnitts begrenzt wird.
...
\begin{enumerate}
    ...
    \item \textbf{Dual-Angle Routing}: Basierend auf dem Rotationswinkel wird entweder der Cardinal Path (bei Winkeln $\leq$ $12^\circ$) oder der Angular Path (bei Winkeln >$12^\circ$) gewählt und die entsprechende Transformation (diskrete Rotation oder Perspektiventransformation) durchgeführt.
...
Datensätze aus den Gleisplänen und adressiert die Anforderungen \hyperref[req:FA-006]{FA-006}
(Fahrtrichtungsdetektion) und \hyperref[req:FA-007]{FA-007} (Symbol-Koordinaten-Verknüpfung).
...
Für die Verknüpfung eines Symbols (z.B. Signal, GKS) mit seiner zugehörigen Koordinatenbeschriftung werden die maximalen Suchentfernungen wie folgt berechnet:

\textbf{Vertikaler Suchbereich:}
\begin{equation}
dy_{\text{max}} = 1{,}6 \cdot h_{\text{symbol}} \cdot k_{dy}
\label{eq:dy_max}
\end{equation}

\textbf{Horizontaler Suchbereich:}
\begin{equation}
dx_{\text{max}} = \max\left( k_{dx} \cdot 0{,}6 \cdot \max(w_{\text{symbol}}, w_{\text{coord}}), \; 30 \right)
\label{eq:dx_max}
\end{equation}

wobei $h_{\text{symbol}}$ die Höhe und $w_{\text{symbol}}$ die Breite der Symbol-Bounding-Box bezeichnen, $w_{\text{coord}}$ die Breite der Koordinaten-Box, und $k_{dy}$, $k_{dx}$ klassenspezifische Multiplikatoren sind. Der Mindestwert von 30 Pixeln für $dx_{\text{max}}$ stellt sicher, dass auch bei sehr schmalen Symbolen ein ausreichender horizontaler Suchbereich gewährleistet ist.

\subsubsection{Klassenspezifische Multiplikatoren}

Die Multiplikatoren wurden empirisch auf Basis der typischen Layoutkonventionen in Gleisplänen optimiert. Tabelle~\ref{tab:linking_multipliers} zeigt die konfigurierten Werte für alle Symbolklassen, wobei die Wertebereiche für $dy_{\text{max}}$ und $dx_{\text{max}}$ auf typischen Symbolgrößen von 50--80 Pixeln bei 500 DPI basieren.

\begin{table}[H]
\centering
\caption{Klassenspezifische Linking-Parameter}
\label{tab:linking_multipliers}
\begin{tabular}{|l|c|c|r|r|}
\hline
\textbf{Klasse} & $\boldsymbol{k_{dy}}$ & $\boldsymbol{k_{dx}}$ & \textbf{$dy_{\text{max}}$} & \textbf{$dx_{\text{max}}$} \\
\hline
coordinate & --- & --- & \multicolumn{2}{c|}{(Verknüpfungsziel)} \\
\hline
signal & 1.0 & 1.0 & 80--130 px & 30--50 px \\
gks\_festkodiert & 1.0 & 1.0 & 80--130 px & 30--50 px \\
gks\_gesteuert & 1.0 & 1.0 & 80--130 px & 30--50 px \\
gm\_block & 1.0 & 1.0 & 80--130 px & 30--50 px \\
haltepunkt & 1.0 & 1.0 & 80--130 px & 30--50 px \\
sverbinder & 1.0 & 1.0 & 80--130 px & 30--50 px \\
isolierstoß & 1.0 & 1.0 & 80--130 px & 30--50 px \\
haltetafel & 1.0 & 2.0 & 80--130 px & 60--100 px \\
prellbock & 1.0 & 2.0 & 80--130 px & 60--100 px \\
weichenende & 1.0 & 3.0 & 80--130 px & 90--150 px \\
weichengruppenende & 1.0 & 4.0 & 80--130 px & 120--200 px \\
\hline
\end{tabular}
\end{table}

Die erhöhten horizontalen Multiplikatoren ($k_{dx} > 1$) für Klassen wie \textit{haltetafel}, \textit{prellbock} und \textit{weichengruppenende} reflektieren deren typische Layoutcharakteristik: Diese Symbole haben häufig horizontal versetzte Beschriftungen, während andere Klassen überwiegend vertikal ausgerichtete Text-Symbol-Relationen aufweisen.

\subsubsection{Spezialfälle mit festen Suchparametern}

Für bestimmte komplexe Verknüpfungsaufgaben, die über die einfache Symbol-Koordinaten-Zuordnung hinausgehen, werden feste Pixelwerte verwendet. Diese wurden empirisch auf Basis der bei 500 DPI typischen physikalischen Abstände kalibriert.
...
\hline
\multicolumn{4}{|c|}{\textit{Fahrtrichtung-Fallback (TIER 2: Gleistopologie-Fallback)}} \\
\hline
Track Ray-Casting & $d_{\text{ray}}$ & 1500 px & Sicherheitslimit für Ray-Casting \\
\hline
...
& $dy_{\text{min}}$ & 30 px & Min. vertikale Separation \\
\hline
\multicolumn{4}{|c|}{\textit{Koordinatenzuordnung}} \\
\hline
...
\begin{itemize}
    \item 250 px $\approx$ 12,7 mm (typischer Abstand zwischen Signal und zugehöriger GKS)
    ...
    \item 1500 px $\approx$ 76,2 mm (Sicherheitslimit für Gleistopologie-Ray-Casting TIER 2)
    \item 300 px $\approx$ 15 mm (Suchradius für Koordinatenzuordnung bei Isolierstoß)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Klassen mit $k_{dx} = 1.0$ (Signal, GKS, GM-Block, etc.):} Text primär \emph{unterhalb} des Symbols erwartet ($dy_{\text{local}} > 0$). Dies entspricht der standardisierten Zeichnungskonvention für Bahnanlagenpläne.

    \item \textbf{Klassen mit erhöhtem $k_{dx}$ (Haltetafel, Prellbock, etc.):} Flexiblere Richtungstoleranz, da Beschriftungen auch seitlich positioniert sein können ($dy_{\text{local}} > 0$ \textbf{oder} $|dx_{\text{local}}| < dx_{\text{max}}$).
\end{itemize}
...
\begin{equation}
dx_{\text{max,tight}} = k_{dx} \cdot 0{,}45 \cdot \max(w_{\text{symbol}}, w_{\text{coord}})
\end{equation}

Zusätzlich wird bei aktivierter Linkssuche (\texttt{search\_left=True}) der horizontale Suchbereich asymmetrisch erweitert:

\begin{equation}
dx_{\text{max,left}} = 1{,}3 \cdot dx_{\text{max}} \quad \text{(falls Koordinate links vom Symbol)}
\end{equation}
...
Die Fahrtrichtung wird durch die Vorzeichenbetrachtung der lokalen y-Koordinate der GKS (gesteuert) relativ zum Signal bestimmt. Hierbei wird die in Abschnitt \ref{sec:Koordinatentransformation} beschriebene Koordinatentransformation angewendet, um die Position der GKS(gesteuert) im lokalen Koordinatensystem des Signals zu ermitteln. Die Entscheidungsregel lautet:
...
\hyperref[req:FA-008]{FA-008} (manuelle Korrektur durch Human-in-the-Loop), \hyperref[req:NFA-004]{NFA-004}
(Robustheit gegenüber fehlerhaften Eingaben) und \hyperref[req:NFA-005]{NFA-005} (Prüfbarkeit
...
\textbf{Validierungsfokus:} Die regelbasierte Validierung umfasst alle 13 Symbolklassen. Jede Klasse besitzt klassenspezifische Validierungsregeln, die auf deren typischen Textformaten und Plausibilitätsanforderungen basieren.
\subsubsection{Dreistufige Validierungsstrategie}

Die Validierung ist hierarchisch in drei Ebenen organisiert, die aufeinander aufbauen und unterschiedliche Aspekte der Datenqualität adressieren (siehe Abbildung~\ref{fig:validation_architecture}). Diese Strukturierung ermöglicht eine systematische Fehleridentifikation von grundlegenden Formatproblemen bis hin zu komplexen semantischen Inkonsistenzen.
...
Für Signalbezeichnungen wird das Regex-Muster \texttt{[A-ZÄÖÜ]\{1,4\}[0-9]\{1,4\}} verwendet, das ein bis vier Großbuchstaben (einschließlich deutscher Umlaute) gefolgt von einer bis vier Ziffern erwartet. Dieses Format entspricht der standardisierten Signalnomenklatur, bei der Buchstaben den Signaltyp oder die Streckenbezeichnung kodieren, während Ziffern die spezifische Signalnummer angeben. Beispiele für gültige Signalbezeichnungen sind A101 oder BHR201.
...
Die Anwendung automatischer Korrekturen erfolgt nicht undifferenziert, sondern wird durch ein konfidenzbasiertes Bewertungssystem gesteuert (siehe Abbildung~\ref{fig:confidence_decision_readable}). Jede potenzielle Korrektur wird mit einem normalisierten Konfidenzwert im Intervall $[0, 1]$ versehen, der die Zuverlässigkeit der Transformation quantifiziert. Dieser Wert wird aus mehreren Faktoren abgeleitet, insbesondere der Eindeutigkeit der Korrektur, der Häufigkeit des identifizierten Fehlermusters und dem Risiko einer Fehlkorrektur.
...
\node[rectangle, draw, fill=yellow!40, minimum width=2cm, minimum height=0.6cm] at (0,0.6) {Warnung};
...
Der Vergleichsalgorithmus verwendet den \textit{Hungarian Algorithm} (Kuhn-Munkres-Algorithmus) zur optimalen Zuordnung von Elementen zwischen zwei Extraktionsmengen $A$ (Altversion) und $B$ (Neuversion). Im Gegensatz zu einfachen mengentheoretischen Operationen ermöglicht dieser Ansatz eine robuste Behandlung von Duplikaten und Elementen ohne eindeutige Identifikatoren.

\textbf{Resultierende Änderungsmengen:}
\begin{align}
\Delta_{\text{add}} &= \{b \in B \mid \nexists\, a \in A: \text{matched}(a, b)\} \quad \text{(hinzugefügte Objekte)} \\
\Delta_{\text{rem}} &= \{a \in A \mid \nexists\, b \in B: \text{matched}(a, b)\} \quad \text{(entfernte Objekte)} \\
\Delta_{\text{mov}} &= \{(a, b) \mid \text{matched}(a, b) \land \text{coord}(a) \neq \text{coord}(b)\} \quad \text{(verschobene Objekte)}
\end{align}

Abbildung~\ref{fig:diff_algorithm} visualisiert diese mengentheoretischen Relationen.
...
\end{figure}
Für erfolgreich zugeordnete Objektpaare (matched) wird geprüft, ob sich der Kilometrierungswert (\texttt{coord\_value}) geändert hat. Ein Objekt gilt als verschoben, falls:

\begin{equation}
\text{isMoved}(o_A, o_B) = \begin{cases}
\text{true}, & \text{falls } |\text{coord}(o_A) - \text{coord}(o_B)| > 0 \\
\text{false}, & \text{sonst}
\end{cases}
\end{equation}
...
\begin{equation}
\text{severity}(\Delta_{\text{coord}}) = \begin{cases}
\text{MINOR}, & \text{falls } \Delta_{\text{coord}} < 5\,\text{m} \\
\text{MODERATE}, & \text{falls } 5\,\text{m} \leq \Delta_{\text{coord}} \leq 20\,\text{m} \\
\text{MAJOR}, & \text{falls } \Delta_{\text{coord}} > 20\,\text{m}
\end{cases}
\end{equation}
...
\hline
signal\_AS102 & Verschoben & km 18.1606 & km 18.1650 ($+4{,}4$\,m) \\
signal\_F5 & Hinzugefügt & $-$ & km 23.4500 \\
gks\_98765 & Entfernt & km 15.2300 & $-$ \\
\hline
\end{tabular}
\end{table}
...
\textbf{Anmerkung zu mehrfachen Einträgen:} Die Tabelle zeigt drei Einträge für GKS 0303 und zwei Einträge für GKS 0305. Dies ist kein Fehler, sondern resultiert aus der Struktur der verglichenen Gleispläne: Diese enthalten drei Streckenabschnitte (Varianten), wobei dieselbe GKS-Kennung in mehreren Abschnitten vorkommen kann. Jede Instanz wird separat verglichen, da sie unterschiedliche Kilometrierungen aufweisen kann.

Diese Darstellung erfüllt Anforderung \hyperref[req:FA-011]{FA-011} und ermöglicht Ingenieuren eine schnelle Identifikation relevanter Planänderungen ohne manuelle Vergleichsarbeit.

Ein konkretes Beispiel einer detektierten Koordinatenänderung zeigen die Abbildungen~\ref{fig:gks_change_before} und~\ref{fig:gks_change_after} für die GKS mit der Kennung 0305.
...
Die vollständige Rückverfolgbarkeit extrahierter Daten zur Originalquelle ist essentiell für die Validierung und manuelle Nachkorrektur von Extraktionsergebnissen. Hierzu wird für jeden extrahierten Datensatz ein umfassender Metadatensatz persistiert, dessen hierarchische Struktur Abbildung~\ref{fig:metadata_structure} zeigt.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
    scale=0.95,
...
\begin{itemize}
    \item ISO 8601~\cite{ISO8601} formatierter Zeitstempel der Extraktion
\end{itemize}
...
Das System verwendet PostgreSQL (Version 14.9) zur persistenten Speicherung von Extraktionsergebnissen, Workspace-Zuständen und Qualitätsmetriken. Das Datenbankschema ist in Abbildung~\ref{fig:database_schema} dargestellt.
\begin{figure}[H]
\centering
...
track_skeleton    TEXT                        -- Komprimierte Gleistopologie
...
\begin{itemize}
    ...
    Persistiert Workspace mit Extraktionsdaten, optionaler Gleistopologie und Bildabmessungen
    
    \item $\texttt{get\_workspace\_data}(name) \rightarrow (data, skeleton, dims)$ \\
    Lädt gespeicherten Workspace inkl. Dekompression der Gleistopologie
\end{itemize}
...
\hyperref[req:FA-013]{FA-013} (GUI ohne CLI-Kenntnisse), \hyperref[req:FA-012]{FA-012} (visuelle Validierung
durch Bounding-Box-Overlays) und \hyperref[req:NFA-001]{NFA-001} (On-Premise-Verarbeitung als
Desktop-Anwendung).
...
\begin{enumerate}
    ...
    (vgl. Anforderung \hyperref[req:NFA-009]{NFA-009}).
    ...
    und Modell erfüllt Anforderung \hyperref[req:NFA-008]{NFA-008} (Update-Fähigkeit) und 
...
Erweiterbarkeit des Systems gemäß Anforderung \hyperref[req:NFA-008]{NFA-008}.

Nach Abschluss der automatischen Analyse wechselt die Anwendung automatisch 
in den Bearbeitungs- und Korrekturmodus (\ref{fig:complete_ui}), 
...
Über dem gerenderten PDF wird eine transparente Grafikschicht gelegt, auf der die erkannten Bounding Boxes (OBBs) als Polygone visualisiert werden. Alle 12 Symbolklassen werden einheitlich in Grün dargestellt, um eine klare visuelle Abgrenzung vom Dokumentinhalt zu gewährleisten. Jede Bounding Box zeigt zusätzlich folgende Informationen als Textlabel an:

\begin{itemize}
    \item Objektbezeichnung (z.\,B. Signalname, GKS-Nummer)
    \item Konfidenzwert der Detektion
    \item Rotationswinkel der OBB
    \item Verknüpfte Koordinate (falls vorhanden)
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Text}: Extrahierter OCR-Text (Objekttext oder Koordinatennummer)
...
\begin{itemize}
    ...
    \item \textbf{Fehlende Verknüpfung}: Symbol ohne zugeordnete Koordinate
...
\subsubsection{Matching-Algorithmus}

Die Zuordnung korrespondierender Elemente zwischen zwei Planversionen erfolgt mittels des \textit{Hungarian Algorithm} (Kuhn-Munkres-Algorithmus), der eine global optimale Zuweisung gewährleistet. Im Gegensatz zu einem Greedy-Ansatz, der bei Duplikaten (mehrere Elemente derselben Klasse an ähnlichen Positionen) zu fehlerhaften Zuordnungen führen kann, findet der Hungarian-Algorithmus die Zuweisung mit maximalem Gesamtscore.

Das Matching erfolgt in zwei Phasen:

\textbf{Phase 1 -- UUID-Matching:} Elemente mit identischer \texttt{detection\_id} werden direkt zugeordnet, da diese eine eindeutige Identifikation garantieren.

\textbf{Phase 2 -- Semantisches Fallback-Matching:} Für nicht-zugeordnete Elemente wird der Hungarian-Algorithmus klassenweise angewendet. Dabei wird eine Kostenmatrix $C$ erstellt, wobei $C_{ij} = -\text{score}(a_i, b_j)$ den negativen Ähnlichkeitsscore zwischen Element $a_i$ der Altversion und Element $b_j$ der Neuversion darstellt.

Die Scoring-Funktion berücksichtigt klassenspezifische Kriterien:

\begin{itemize}
    \item \textbf{OCR-Klassen} (Signal, GKS): Primär Textähnlichkeit des Symboltextes, ergänzt durch Koordinaten- und Positionsboni. Kein Distanz-Cutoff, da eindeutige Bezeichner vorliegen.
    \item \textbf{Symbol-Klassen} (Sverbinder, GM-Block, etc.): Primär Koordinatenwert-Übereinstimmung mit hartem Cutoff bei $\Delta_{\text{coord}} > 100$\,m, da keine eindeutigen Bezeichner existieren.
\end{itemize}

Eine Zuordnung wird akzeptiert, wenn der Score den Schwellenwert $\theta = 0{,}7$ überschreitet. Nicht zugeordnete Elemente der Altversion gelten als \textit{gelöscht}, nicht zugeordnete Elemente der Neuversion als \textit{hinzugefügt}.

Ein Element gilt als \textit{verschoben}, wenn sich dessen Kilometrierungswert (\texttt{coord\_value}) zwischen den Versionen geändert hat. Die Schwere der Verschiebung wird nach Distanz klassifiziert: geringfügig ($< 5$\,m), moderat ($5$--$20$\,m) oder signifikant ($> 20$\,m).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/Kapitel6/diff_dialog.png}\caption{Änderungsvergleich-Dialog}
...
kategorisiert die Ergebnisse in drei Änderungstypen: \textit{Hinzugefügt} (neue Elemente in
PDF~2), \textit{Gelöscht} (fehlende Elemente in PDF~2) und \textit{Verschoben} (gleiche
Kennung, unterschiedliche Kilometrierung). Zusätzlich werden \textit{Unveränderte} Elemente angezeigt. Im dargestellten Beispiel wurden zwischen zwei Versionen 
des Plans insgesamt 41 Änderungen erkannt: 21 hinzugefügte, 15 gelöschte 
und 5 verschobene Elemente. Die einzelnen Tabs 
...
und die Anforderungen \hyperref[req:FA-009]{FA-009} (Excel-Integration) sowie \hyperref[req:FA-010]{FA-010} (Strukturerhalt) 
adressiert. Die wichtigsten Funktionen umfassen:

\begin{itemize}
    \item \textbf{Vorlagen:} Zwei vordefinierte Spaltenauswahlen für unterschiedliche Anwendergruppen:
    \begin{itemize}
        \item \textit{Techniker}: Reduzierte Ansicht mit Klassenname, Koordinate und Fahrtrichtung -- optimiert für Bahntechniker, die nur die betriebsrelevanten Daten benötigen
        \item \textit{Entwickler}: Vollständige Ansicht mit allen technischen Details (Bounding-Box-Koordinaten, Rotationswinkel, Konfidenzwerte) -- für KI-Entwickler zur Modellvalidierung
    \end{itemize}
...
Funktionalität erfüllt die Anforderung \hyperref[req:FA-010]{FA-010} (Strukturerhalt).
...
Diese Struktur ermöglicht die vollständige Rekonstruktion der Extraktionsergebnisse und eignet sich für Weiterverarbeitung in anderen Systemen. Abbildung~\ref{fig:export_format_selection} zeigt den Export-Dialog, dessen Dropdown-Menü drei Ausgabeformate bietet: Excel (.xlsx), CSV (.csv) und JSON (.json).
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/Kapitel6/export_dialog_json.png}
\caption{Export-Dialog mit Formatauswahl}
\label{fig:export_format_selection}
\end{figure}

Gemäß Anforderung \hyperref[req:NFA-010]{NFA-010} unterstützt das System drei Ausgabeformate:
