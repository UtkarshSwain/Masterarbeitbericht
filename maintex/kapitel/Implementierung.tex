...
\hline
\textbf{Klasse} & \textbf{Train Bilder} & \textbf{Train Inst.} & \textbf{Val Bilder} & \textbf{Val Inst.} & \textbf{Gesamt Inst.} \\
\hline
...
Das Training wurde auf einer cloud-basierten GPU-Instanz mit einer NVIDIA-GPU und 16~GB Videospeicher durchgeführt. Das Modell wurde mit auf dem COCO-Datensatz \cite{coco} vortrainierten Gewichten initialisiert (\textit{Transfer Learning}), um die in den frühen Netzwerkschichten bereits gelernten generischen Merkmalsextraktoren -- wie Kanten-, Ecken- und Texturerkennung -- zu nutzen und die Konvergenz auf dem domänenspezifischen Gleisplan-Datensatz zu beschleunigen.
...
\hline
\end{tabular}
\caption{Hyperparameter des YOLOv8s-OBB-Trainings}
...
Abbildung~\ref{fig:confusion_matrix} zeigt die normalisierte Konfusionsmatrix auf dem Validierungsdatensatz. Die Diagonale zeigt durchgehend hohe Werte ($\geq 0{,}93$), was eine zuverlässige Klassifikation über alle Klassen bestätigt.
...
Die wesentlichen Fehlerquellen lassen sich wie folgt zusammenfassen:

\begin{itemize}
    \item \textbf{GKS-Verwechslung:} Die Klasse \textit{gks\_festkodiert} weist mit 93\,\% die niedrigste Recall-Rate auf, wobei 3\,\% der Instanzen als \textit{gks\_gesteuert} fehlklassifiziert und 1\,\% als Hintergrund übersehen werden. Diese bidirektionale Verwechslung ist nachvollziehbar, da beide GKS-Varianten eine ähnliche visuelle Morphologie aufweisen und sich primär durch interne Markierungsdetails unterscheiden, die bei kleinen Bounding-Box-Ausschnitten schwer differenzierbar sind.
    
    \item \textbf{Isolierstoß-Erkennung:} Die Klasse \textit{isolierstoß} erreicht mit 93\,\% die gemeinsam niedrigste Recall-Rate, wobei 7\,\% der Instanzen als Hintergrund übersehen werden. Dies ist auf die geringe Symbolgröße zurückzuführen, die insbesondere an Tile-Grenzen zu unvollständigen Detektionen führen kann.
    
    \item \textbf{Hintergrund-Fehlzuordnungen:} Die Spalte \textit{background} zeigt, dass das Modell vereinzelt Hintergrundstrukturen fälschlicherweise als Symbole interpretiert. Die höchsten False-Positive-Raten betreffen \textit{coordinate} (0,32) und \textit{signal} (0,20), was angesichts der hohen absoluten Instanzzahlen dieser Klassen jedoch zu relativieren ist.
\end{itemize}
