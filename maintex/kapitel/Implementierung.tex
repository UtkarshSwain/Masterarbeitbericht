...
Die Spalte \textit{Bilder} zeigt die Anzahl der Bilder mit mindestens einer Instanz
der jeweiligen Klasse; da Bilder mehrere Klassen gleichzeitig enthalten, ist die Summe
der Einzelwerte größer als die mit~\textsuperscript{*} markierte Gesamtbildzahl.}
...
\begin{figure}[H]
    ...
    (oben links)~Klassenverteilung mit absoluten Instanzzahlen, 
    (unten links)~räumliche Verteilung der Bounding-Box-Zentren,
    (unten rechts)~Größenverteilung der annotierten Objekte in normalisierten Koordinaten.}
...
Das Modell wurde mit auf dem COCO-Datensatz \cite{Lin2014_COCO}
...
\begin{enumerate}
    \item \textbf{Warmup-Phase (Epochen 1--3):} Die Lernrate wird linear auf den Initialwert hochgefahren. Alle Verlustkomponenten fallen steil ab, was die schnelle Adaption der vortrainierten COCO-Gewichte an die Gleisplan-Domäne zeigt. Bereits nach Epoche~3 erreicht das Modell eine mAP@0,5 von 92,5\,\%.
...
Abbildung~\ref{fig:pr_curve} zeigt die Precision-Recall-Kurven für alle 13~Symbolklassen. Die klassenspezifischen AP@0,5-Werte sind in Tabelle~\ref{tab:per_class_ap} zusammengestellt.
...
\begin{figure}[H]
    ...
    Die Werte in der Legende geben die jeweilige AP@0,5 an.}
    \label{fig:pr_curve}
\end{figure}
...
\begin{figure}[H]
    ...
    Zeilen repräsentieren die vorhergesagten Klassen, Spalten die tatsächlichen Klassen.}
    \label{fig:confusion_matrix}
\end{figure}
\newpage
\paragraph{Konfidenzkalibrierung.}
Abbildung~\ref{fig:f1_curve} zeigt die F1-Score-Konfidenz-Kurve für alle 13~Klassen.
...
\begin{figure}[H]
    ...
    Das globale Optimum liegt bei $F1 = 0{,}96$ bei einer Schwelle von 0,674.}
    \label{fig:f1_curve}
\end{figure}
