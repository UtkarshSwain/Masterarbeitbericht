...
Der finale Datensatz umfasst insgesamt \textbf{13~Symbolklassen} gemäß Anforderung \textbf{FA-003}: \textit{coordinate}, \textit{signal}, \textit{gks\_festkodiert}, \textit{gks\_gesteuert}, \textit{gm\_block}, \textit{sverbinder}, \textit{haltepunkt}, \textit{weichen\_block}, \textit{isolierstoß}, \textit{prellbock}, \textit{haltetafel}, \textit{endeweichen} und \textit{weichengruppeende}. Die Klasse \textit{coordinate} nimmt eine besondere Rolle ein: Sie liefert die Positionsinformation (Kilometrierung), die den anderen 12~Klassen zugeordnet wird. Alle 13~Klassen werden in Kapitel~\ref{chap:evaluation} quantitativ evaluiert.
...
\begin{enumerate}
    \item \textbf{Klassenungleichgewicht:} Die natürliche Verteilung der Symbolklassen in den 482~annotierten Original-Tiles ist stark unausgeglichen. Häufige Klassen wie \textit{coordinate} dominieren, während seltene Klassen wie \textit{endeweichen} oder \textit{weichengruppeende} nur in wenigen Tiles vorkommen. Ohne Gegenmaßnahmen besteht die Gefahr, dass das Modell auf häufige Klassen overfittet und seltene Symbole unzureichend lernt.
...
\hline
\end{tabular}
\caption{Aggregierte Detektionsmetriken auf dem Validierungsdatensatz (208~Bilder)}
...
Bemerkenswert ist, dass \textbf{alle 13~Klassen eine AP@0.5 über 95\,\%} erreichen und die Streuung mit 95,1\,\% bis 99,5\,\% sehr gering ist. Dies bestätigt, dass die in Abschnitt~\ref{subsubsec:augmentation} beschriebene Augmentationsstrategie das Klassenungleichgewicht erfolgreich adressiert hat: Auch Klassen mit wenigen Validierungsinstanzen (z.\,B.\ \textit{weichengruppeende} mit nur 18~Instanzen) erreichen eine AP@0.5 von 99,0\,\%.
...
Für die Verknüpfung eines Ankersymbols (z.B. Signal, GKS) mit seiner zugehörigen Koordinatenbeschriftung werden die maximalen Suchentfernungen wie folgt berechnet:
...
\hline
\textbf{Klasse} & $\boldsymbol{k_{dy}}$ & $\boldsymbol{k_{dx}}$ & \textbf{$dy_{\text{max}}$} & \textbf{$dx_{\text{max}}$} & \textbf{Rolle} \\
\hline
coordinate & --- & --- & \multicolumn{2}{c|}{(Verknüpfungsziel)} & Koordinate \\
\hline
signal & 1.0 & 1.0 & 80--130 px & 30--50 px & Anker \\
gks\_festkodiert & 1.0 & 1.0 & 80--130 px & 30--50 px & Anker \\
gks\_gesteuert & 1.0 & 1.0 & 80--130 px & 30--50 px & Anker \\
gm\_block & 1.0 & 1.0 & 80--130 px & 30--50 px & Anker \\
...
\begin{enumerate}
    ...
    und Modell erfüllt Anforderung \textbf{NFA-008} (Update-Fähigkeit) und 
...
\begin{itemize}
    \item \textbf{OCR-Klassen} (Signal, GKS): Primär Textähnlichkeit des Ankertextes, ergänzt durch Koordinaten- und Positionsboni. Kein Distanz-Cutoff, da eindeutige Bezeichner vorliegen.
...
Gemäß Anforderung \textbf{NFA-010} unterstützt das System drei Ausgabeformate:
...
Das YOLOv8-OBB Modell wurde auf einem Datensatz mit 13 Symbolklassen trainiert (vgl. Anforderung FA-003). Die Klasse \textit{coordinate} liefert Positionsinformationen, die den anderen 12~Klassen zugeordnet werden. Alle 13~Klassen werden in Kapitel~\ref{chap:evaluation} quantitativ evaluiert.
