...
Die initialen Gleispläne lagen im PDF-Format vor und mussten zunächst in ein für die Annotation geeignetes Bildformat überführt werden. Hierzu wurde die Python-Bibliothek PyMuPDF eingesetzt, welche eine zuverlässige Rasterisierung von PDF-Dokumenten ermöglicht. Die Konvertierung erfolgte mit einer Auflösung von 500 DPI (dots per inch), was einem Skalierungsfaktor von $\frac{500}{72} \approx 6.94$ gegenüber der Standard-PDF-Auflösung entspricht. Diese hohe Auflösung wurde gewählt, um auch kleine Symbole wie Gleiskoppelspulen oder Isolierstöße mit ausreichender Detailgenauigkeit darzustellen.

Nach der Rasterisierung wurden die hochauflösenden Bilder in kleinere, überlappende Bildausschnitte segmentiert. Die gewählte Segmentgröße für die Annotation betrug $2048 \times 2048$ Pixel, um ausreichend Kontext für die präzise Markierung von Symbolen und deren räumliche Beziehungen zu gewährleisten. Für das Training und die Inferenz werden diese Tiles auf $1024 \times 1024$ Pixel herunterskaliert (Downsampling), was der YOLO-Eingabegröße entspricht. Diese Strategie kombiniert die Vorteile großflächiger Annotation mit effizienter Modellverarbeitung: Die Annotationen auf 2048×2048 Pixeln erfassen mehr Kontextinformation, während das Downsampling die Rechenzeit signifikant reduziert -- ein Tile mit $2048 \times 2048$ Pixeln enthält viermal so viele Pixel wie eines mit $1024 \times 1024$, was zu einer entsprechend längeren Inferenzzeit führt. Zudem reduziert die kleinere Eingabegröße den GPU-Speicherbedarf während des Trainings erheblich, was größere Batch-Größen und damit stabilere Gradientenschätzungen ermöglicht. Bei ausreichender Symbolauflösung (die bei 500 DPI Renderauflösung gegeben ist) bleiben die relevanten visuellen Merkmale auch nach dem Downsampling differenzierbar. Diese Dimensionierung stellt somit einen Kompromiss dar zwischen Annotationsqualität und effizienter Verarbeitung auf GPU-Systemen mit begrenztem Videospeicher (NVIDIA, AWS Instance g4dn.xlarge, 16 GB VRAM).
...
Der finale Datensatz umfasst insgesamt 13 Symbolklassen, darunter Signale, verschiedene Weichentypen, Koordinatenbeschriftungen, Haltepunkte, Isolierstöße und weitere eisenbahnspezifische Elemente. Die Daten wurden in einen Trainingssatz und einen Validierungssatz aufgeteilt, wobei der Validierungssatz 115 Bilder mit insgesamt 1.305 annotierten Objektinstanzen enthält.

Die Verteilung der Objektinstanzen über die Klassen hinweg zeigt Tabelle~\ref{tab:dataset_distribution}. Die Spalte \textit{Bilder} gibt dabei an, in wie vielen der 115 Validierungsbilder mindestens eine Instanz der jeweiligen Klasse vorkommt. Da ein einzelnes Bild typischerweise mehrere Symbolklassen gleichzeitig enthält, überlappen sich diese Werte und können nicht zur Gesamtzahl von 115 Bildern aufsummiert werden. Die Spalte \textit{Instanzen} hingegen zeigt die absolute Anzahl der annotierten Objekte pro Klasse, deren Summe die Gesamtzahl von 1.305 Annotationen ergibt.
...
\hline
\end{tabular}
\caption{Verteilung der Annotationen über Klassen und Bilder im Validierungssatz, unterteilt nach Kern- und Auxiliarklassen (vgl. Anforderung FA-003). *Die Spalte \textit{Bilder} zeigt die Anzahl der Bilder mit mindestens einer Instanz der jeweiligen Klasse; da Bilder mehrere Klassen enthalten können, ist die Summe der Einzelwerte größer als die Gesamtbildzahl.}
...
Eingabeauflösung & $1024 \times 1024$ & Downsampling von 2048×2048 Annotationen; 4× Reduktion der Pixelanzahl für effiziente Verarbeitung \\
...
Die erhöhten horizontalen Multiplikatoren ($k_{dx} > 1$) für Auxiliarklassen wie \textit{haltetafel}, \textit{prellbock} und \textit{weichengruppeende} reflektieren deren typische Layoutcharakteristik: Diese Symbole haben häufig horizontal versetzte Beschriftungen, während die Kernklassen überwiegend vertikal ausgerichtete Text-Symbol-Relationen aufweisen.
