...
OCR (Fallback 1) & Tesseract & 5.3.0 & Fallback für pixelige Texte \\
...
\begin{itemize}
    ...
    \item \texttt{config.py} - Konfigurationsmanagement
    \item \texttt{main.py} - Haupteinstiegspunkt
\end{itemize}

Die Modularität ermöglicht eine unabhängige Weiterentwicklung einzelner Komponenten sowie eine einfache Integration neuer Funktionen oder OCR-Engines.
...
Abschnitt 3.3.2), repräsentiert durch Zentrum $(c_x, c_y)$, Dimensionen 
...
\hline
\end{tabular}
\caption{Verteilung der Annotationen über Klassen und Bilder im Validierungssatz}
\label{tab:dataset_distribution}
\end{table}

Es ist ersichtlich, dass die Klassenverteilung unausgeglichen ist, mit Koordinaten und Signalen als dominierende Klassen. Diese Verteilung spiegelt die reale Häufigkeit der Symbole in Gleisplänen wider. Um ein Overfitting auf häufige Klassen zu vermeiden, wurde während des Trainings eine klassengewichtete Verlustfunktion in Betracht gezogen, letztendlich jedoch zugunsten einer robusten Datenaugmentation verworfen.
...
Workers & 2 & Moderat gewählt zur Vermeidung von Dataloader-OOM \\
...
\hline
Rotation & 0.0° & Deaktiviert, da OBB bereits Rotationsinvarianz bietet \\
...
Scherung & 0.0° & Deaktiviert, um geometrische Integrität zu bewahren \\
...
\hline
\textbf{Klasse} & \textbf{Precision} & \textbf{Recall} & \textbf{mAP@0.5} & \textbf{mAP@0.5:0.95} \\
\hline
signal & 1.000 & 0.985 & 0.995 & 0.915 \\
gm\_block & 0.909 & 1.000 & 0.989 & 0.886 \\
gks\_festkodiert & 0.976 & 0.983 & 0.988 & 0.974 \\
gks\_gesteuert & 0.941 & 1.000 & 0.958 & 0.931 \\
weichen\_block & 0.963 & 0.974 & 0.990 & 0.914 \\
isolierstoß & 0.988 & 1.000 & 0.995 & 0.967 \\
haltepunkt & 0.921 & 0.991 & 0.968 & 0.867 \\
sverbinder & 0.928 & 1.000 & 0.979 & 0.932 \\
coordinate & 0.971 & 0.968 & 0.989 & 0.926 \\
prellblock & 1.000 & 1.000 & 0.995 & 0.901 \\
haltetafel & 0.964 & 0.818 & 0.959 & 0.919 \\
endeweichen & 0.909 & 1.000 & 0.995 & 0.904 \\
weichengruppeende & 0.966 & 1.000 & 0.995 & 0.966 \\
\hline
\textbf{Durchschnitt} & \textbf{0.957} & \textbf{0.979} & \textbf{0.984} & \textbf{0.923} \\
\hline
\end{tabular}
\caption{Klassenspezifische Detektionsmetriken auf dem Validierungssatz}
\label{tab:yolo_class_metrics}
\end{table}

Die Precision gibt an, welcher Anteil der vom Modell vorhergesagten Detektionen korrekt ist:

\begin{equation}
\text{Precision} = \frac{TP}{TP + FP}
\end{equation}

wobei $TP$ (True Positives) die Anzahl korrekter Detektionen und $FP$ (False Positives) die Anzahl fälschlicherweise detektierter Objekte bezeichnet.

Der Recall misst, welcher Anteil der tatsächlich vorhandenen Objekte vom Modell erkannt wurde:

\begin{equation}
\text{Recall} = \frac{TP}{TP + FN}
\end{equation}

mit $FN$ (False Negatives) als Anzahl der übersehenen Objekte.

Die durchschnittliche Precision von 95.7\% zeigt, dass das Modell äußerst wenige Falschdetektionen produziert. Der noch höhere Recall von 97.9\% belegt, dass nahezu alle vorhandenen Symbole erkannt werden. Die Kombination dieser beiden Werte deutet auf ein ausgewogenes Modell hin, das weder zu aggressiv (hoher Recall, niedrige Precision) noch zu konservativ (hohe Precision, niedriger Recall) detektiert.

Der mean Average Precision (mAP) ist eine aggregierte Metrik, die die Precision über verschiedene Recall-Stufen mittelt und zusätzlich über alle Klassen aggregiert. Der mAP@0.5 von 98.4\% bezieht sich auf eine IoU-Schwelle (Intersection over Union) von 50\%, während der mAP@0.5:0.95 den Durchschnitt über IoU-Schwellen von 50\% bis 95\% in 5\%-Schritten bildet. Der Unterschied zwischen diesen beiden Werten (98.4\% vs. 92.3\%) deutet darauf hin, dass die Bounding-Box-Lokalisierung bei sehr hohen IoU-Anforderungen ($>$ 0.75) leicht abnimmt, was für die praktische Anwendung jedoch unkritisch ist, da bereits eine IoU von 0.5 als ausreichend präzise gilt.

\subsubsection{Konvergenzverhalten}

Die Trainingsverläufe (Abbildung \ref{fig:yolo_training_curves}) illustrieren die Entwicklung der Verlustfunktionen und Metriken über die 120 Epochen:

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/Kapitel6/results.png}
\caption{Trainings- und Validierungsmetriken über 120 Epochen. Die Kurven zeigen eine stabile Konvergenz ohne Anzeichen von Overfitting.}
...
\begin{itemize}
    \item Die Box-Loss-Kurve, welche die Lokalisierungsgenauigkeit der Bounding Boxes quantifiziert, fällt kontinuierlich und stabilisiert sich nach ca. 80 Epochen auf einem niedrigen Niveau. Dies indiziert, dass das Modell gelernt hat, Objekte präzise zu lokalisieren.
    
    \item Die Classification-Loss-Kurve zeigt einen ähnlichen Verlauf, was darauf hindeutet, dass die Klassifikation der detektierten Objekte ebenfalls erfolgreich konvergiert.
    
    \item Precision und Recall erreichen bereits nach ca. 50 Epochen Plateaus bei 95-97\% und verbessern sich danach nur noch marginal. Dies zeigt, dass das Modell die wesentlichen Muster früh im Training lernt und die verbleibenden Epochen zur Feinabstimmung dienen.
    
    \item Der Distribution Focal Loss (DFL), eine spezialisierte Verlustfunktion für die Regression der Bounding-Box-Koordinaten in YOLO, zeigt eine stabile Konvergenz ohne Oszillationen.
    
    \item Kritisch ist die Beobachtung, dass zwischen Trainings- und Validierungskurven kein signifikanter Gap existiert. Dies belegt, dass das Modell gut generalisiert und nicht auf den Trainingsdaten overfittet. Wäre dies der Fall, würde die Trainingskurve deutlich bessere Werte aufweisen als die Validierungskurve.
\end{itemize}

Die Konfusionsmatrix (Abbildung \ref{fig:confusion_matrix}) visualisiert die Klassifikationsgenauigkeit und offenbart, zwischen welchen Klassen das Modell gelegentlich verwechselt:


\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/Kapitel6/confusion_matrix.png}
\caption{Konfusionsmatrix des trainierten YOLOv8-OBB Modells. Die diagonale Dominanz zeigt hohe Klassifikationsgenauigkeit; Off-Diagonal-Einträge deuten auf gelegentliche Verwechslungen hin.}
\label{fig:confusion_matrix}
\end{figure}



Die Matrix zeigt eine ausgeprägte Diagonalstruktur, was darauf hinweist, dass die meisten Detektionen der korrekten Klasse zugeordnet werden. Vereinzelte Off-Diagonal-Einträge treten hauptsächlich zwischen visuell ähnlichen Symboltypen auf, beispielsweise zwischen \textit{gks\_festkodiert} und \textit{gks\_gesteuert}, was aufgrund ihrer hohen visuellen Ähnlichkeit erwartbar ist. Ebenfalls tritt eine geringe Konfusion zwischen \textit{coordinate} und Hintergrund auf, was bei dicht gruppierten oder schwach lesbaren Koordinatenbeschriftungen zu erwarten ist.

\subsection{Inferenz-Pipeline Implementierung}
\label{Inferenz}
Nach Abschluss des Trainings wurde das Modell mit den optimalen Gewichten (\texttt{best.pt}) in die produktive Extraktionspipeline integriert. Die Inferenz auf neuen, zuvor ungesehenen Gleisplänen erfolgt in mehreren aufeinander aufbauenden Schritten.
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}[
    ...
    \end{tikzpicture}
    \caption{Ablauf der Inferenz-Pipeline: Von der PDF-Eingabe über das Tiling bis zur Rücktransformation der globalen Koordinaten.}
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{2D-Tiling-Strategie mit quadratischen Kacheln ($H=W$). Durch die identische Schrittweite (Stride) in X- und Y-Richtung wird eine gleichmäßige Abdeckung mit definiertem Überlappungsbereich gewährleistet.}
...
Zusätzlich zur Überlappung wird ein sogenannter Halo um jede Tile herum hinzugefügt. Der sogenannte Halo bezeichnet einen zusätzlichen Randbereich von $H_{\text{halo}} = 128$ Pixeln, der bei der Bildextraktion mitgeladen wird:
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{Visualisierung der Halo-Strategie: Das neuronale Netz erhält einen erweiterten Bildausschnitt (Grau), um angeschnittene Objekte am Rand (S) korrekt zu erkennen. Gewertet werden jedoch nur Detektionen, deren Zentrum im inneren Bereich (Grün) liegt.}
...
\text{Horizontal}, & \text{falls } |\theta_{\text{norm}}| < 15° \text{ oder } |\theta_{\text{norm}} - 180°| < 15° \\
\text{Vertikal}, & \text{falls } |\theta_{\text{norm}} - 90°| < 15° \text{ oder } |\theta_{\text{norm}} - 270°| < 15° \\
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{Adaptive Expansion der Region of Interest (ROI). Im dritten Fall (rechts) ist zu sehen, dass die Expansion entlang der rotierten Objektachsen erfolgt.}
...
Die Kaskadierung funktioniert nach einem sequentiellen Prinzip. Zunächst wird PaddleOCR auf dem Bildausschnitt ausgeführt, und zwar viermal mit unterschiedlichen Rotationswinkeln: 0°, 90°, 180° und 270°. Der Grund für diese vier Rotationen ist, dass PaddleOCR am besten mit horizontal ausgerichtetem Text funktioniert. Durch das Ausprobieren aller vier Hauptrichtungen wird sichergestellt, dass mindestens eine Rotation den Text horizontal ausrichtet.
...
Zunächst wird der von YOLO gelieferte Winkel normalisiert. Da YOLO Winkel im Bereich von 0° bis 360° liefern kann, werden Winkel über 180° durch Subtraktion von 360° in den Bereich -180° bis 180° transformiert. Ein Winkel von 270° wird beispielsweise zu -90°. Dies vereinfacht die Logik erheblich, da nun positive Winkel eine Drehung gegen den Uhrzeigersinn und negative Winkel eine Drehung im Uhrzeigersinn darstellen.

Anschließend wird geprüft, ob der Absolutwert des normalisierten Winkels unter 15° liegt. Diese 15° wurden empirisch durch Experimente bestimmt: Bei Winkeln bis zu dieser Größe liefert eine einfache Rotation auf den nächsten kardinalen Winkel (0°, 90°, 180° oder 270°) bessere Ergebnisse als eine komplexe Perspektiventransformation. Der Grund ist, dass bei kleinen Winkeln die Perspektiventransformation zu leichten Verzerrungen führen kann, während die diskrete Rotation verlustfrei ist.

\textbf{Cardinal Path (Winkel $\leq$ 15°):} Wenn der Winkel nahe an einem kardinalen Wert liegt, wird der Bildausschnitt einfach auf den nächstgelegenen kardinalen Winkel rotiert. Die Rotation erfolgt mittels einer Standard-Rotationsmatrix, die das Bild um den Mittelpunkt dreht. Diese Methode ist schnell, einfach und vermeidet Interpolationsartefakte, da nur 90°-Schritte verwendet werden.

Konkret wird aus den vier Kandidaten (0°, 90°, 180°, 270°) derjenige gewählt, der die kleinste Winkeldifferenz zum normalisierten Winkel hat. Ein Winkel von 12° wird also auf 0° rotiert, ein Winkel von 85° auf 90°, und so weiter.

\textbf{Angular Path (Winkel > 15°):} Bei größeren Abweichungen von den kardinalen Winkeln wird eine Perspektiventransformation verwendet. Diese Methode nutzt die vier Eckpunkte der Oriented Bounding Box (OBB), um eine Transformationsmatrix zu berechnen. Die Transformation wandelt die gedrehte OBB in ein achsenparalleles Rechteck um, wodurch der Text horizontal ausgerichtet wird.

Die Perspektiventransformation wird mit OpenCV's \texttt{getPerspectiveTransform()}-Funktion implementiert. Diese Funktion benötigt zwei Sätze von je vier Punkten: die Quellpunkte (die vier Ecken der gedrehten OBB) und die Zielpunkte (die vier Ecken eines aufrechten Rechtecks). Aus diesen acht Punkten berechnet die Funktion eine 3×3-Transformationsmatrix, die dann mit \texttt{warpPerspective()} auf das Bild angewendet wird.
...
\begin{scope}[yshift=-4cm] 
    ...
    \node[below=0.7cm] at (0,0) {Original};
    \node[below=1.1cm] at (0,0) {$\theta = 35^\circ$};
    ...
    \draw[result] (4.5, -0.5) rectangle (6.5, 0.5);
    \node at (5.5, 0) {Text};
    \node[below=0.7cm] at (5.5,0) {Transformiert};
    \node[below=1.1cm] at (5.5,0) {Ausgerichtet};
\end{scope}
...
Verschiedene Symbolklassen im Gleisplan haben unterschiedliche visuelle Eigenschaften, die spezielle Vorverarbeitungsschritte erfordern. Signale haben beispielsweise oft einen farbigen Hintergrund und größere Beschriftungen, während Balisen häufig von dünnen Linien umgeben sind und sehr kompakte Texte haben. Um diese Unterschiede zu berücksichtigen, wurde für jede Klasse eine spezialisierte Verarbeitungsstrategie implementiert.
...
Die Größe des Paddings wird nicht absolut (z.B. \enquote{10 Pixel}), sondern relativ zur Größe der Bounding Box berechnet. Dies stellt sicher, dass auch bei unterschiedlichen Skalierungen der Gleispläne das Padding angemessen ist. Die Berechnung erfolgt durch:

\begin{equation}
p_x = \alpha \cdot w_{\text{bbox}}, \quad p_y = \beta \cdot h_{\text{bbox}}
\end{equation}
...
Balise & 0.15 & 0.15 & Kompakte, zentrierte Beschriftung \\
...
Für Signale wurde beispielsweise ein Faktor von $\alpha = 0.2$ (20\% der Breite) und $\beta = 0.3$ (30\% der Höhe) als optimal ermittelt. Das größere vertikale Padding berücksichtigt, dass Signal-Beschriftungen typischerweise oberhalb des Symbols positioniert sind. Bei einer Bounding Box von 100×50 Pixeln würde dies ein Padding von 20 Pixeln horizontal und 15 Pixeln vertikal bedeuten.
...
\subsubsection{Morphologische Filterung für Balisen}

Balisen-Symbole sind häufig von dünnen Linien umgeben, die die OCR-Qualität beeinträchtigen können. Diese Linien können fälschlicherweise als Buchstaben wie \enquote{I}, \enquote{l} oder \enquote{1} interpretiert werden. Um diese Störungen zu reduzieren, wird ein morphologischer Opening-Filter angewendet.
...
\end{tikzpicture}
\caption{Morphologisches Opening zur Rauschentfernung bei Balisen-Symbolen}
...
Um dies zu kompensieren, wird ein Multi-Scale-Ansatz verwendet: Das Bild wird mit drei verschiedenen Skalierungsfaktoren vergrößert – 1.5× (50\% größer), 2.0× (doppelte Größe) und 2.5× (2.5-fache Größe). Die OCR wird auf allen drei Skalen durchgeführt, und am Ende wird das Ergebnis mit der höchsten Konfidenz ausgewählt.
...
% Balise symbol (trapezoid) - ohne störende Linien
...
% Balise symbol (trapezoid) - besserer Kontrast
...
% Balise symbol (trapezoid) - vergrößert
...
\begin{enumerate}
    ...
    \item \textbf{Vertikale Linienentfernung}: Analog wird ein vertikales strukturierendes Element (ebenfalls 25 Pixel lang) verwendet, das um 90° zusätzlich gedreht wird, um senkrechte Linien zu entfernen.
...
CLAHE arbeitet anders als globale Histogram-Entzerrung: Das Bild wird in ein Raster von 8×8 Kacheln (Tiles) aufgeteilt. Für jede Kachel wird separat eine Histogram-Entzerrung durchgeführt, die dunkle Bereiche aufhellt und helle Bereiche abdunkelt. Der entscheidende Unterschied: Für jede Kachel wird der Kontrast \textit{lokal} optimiert, nicht global.
...
Jede Symbolklasse hat ein spezifisches Format, das durch Regular Expressions (Regex) beschrieben wird. Diese Patterns definieren, welche Zeichenfolgen für eine Klasse gültig sind.
...
Balise & \verb|^BAL\d{4}$| oder \verb|^\d{4}$| & BAL1234, 5678 \\
Weiche & \verb|^W\d{2,4}[a-z]?$| & W12, W234, W45a \\
Koordinate & \verb|^\d+[+-]\d+$| & 123+456, 78-90 \\
Haltepunkt & \verb|^[A-Z]{2,4}$| & AB, WXYZ \\
\hline
\end{tabular}
\caption{Validierungs-Patterns und Beispiele pro Symbolklasse}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Klassenspezifisches Preprocessing}: Je nach Symbolklasse werden die entsprechenden Vorverarbeitungsschritte angewendet – adaptive Padding-Berechnung, morphologische Filterung für Balisen, oder Multi-Scale-Processing für Koordinaten.
    
    \item \textbf{Dual-Angle Routing}: Basierend auf dem Rotationswinkel wird entweder der Cardinal Path (bei Winkeln $\leq$ 15°) oder der Angular Path (bei Winkeln >15°) gewählt und die entsprechende Transformation (diskrete Rotation oder Perspektiventransformation) durchgeführt.
...
Nach der erfolgreichen Detektion von Symbolen durch YOLOv8-OBB und der Extraktion von Textinhalten mittels der orientierungsadaptiven OCR-Pipeline besteht die zentrale Herausforderung in der korrekten Zuordnung der extrahierten Texte zu ihren zugehörigen Symbolen. Diese Verknüpfung ist essenziell für die Generierung semantisch korrekter Datensätze aus den Gleisplänen.

Die naive Anwendung rein distanzbasierter Proximity-Algorithmen -- bei denen einfach der nächstgelegene Text einem Symbol zugeordnet wird -- führt bei rotierten Symbolen zu systematischen Fehlzuordnungen. Ein Signal, das beispielsweise um 45° gedreht ist, besitzt eine eigene lokale Orientierung, und der zugehörige Text befindet sich typischerweise \enquote{unterhalb} des Signals in dessen lokaler Ausrichtung, nicht notwendigerweise in globalen Bildkoordinaten. Die Implementierung muss daher rotationsinvariante Algorithmen einsetzen, die geometrische Transformationen nutzen, um die semantische räumliche Beziehung zwischen Symbolen und Texten korrekt zu erfassen.
...
Die Herausforderung liegt darin, dass die von YOLO detektierten Symbole beliebige Rotationswinkel $\theta \in [0°, 360°)$ aufweisen können. Die Textpositionen werden in einem globalen kartesischen Koordinatensystem $(x_{\text{global}}, y_{\text{global}})$ angegeben, welches üblicherweise mit der Bildebene übereinstimmt. Für ein rotiertes Symbol ist jedoch die Definition von Richtungen wie \enquote{unterhalb}, \enquote{rechts von} oder \enquote{links von} in globalen Koordinaten nicht mehr sinnvoll.
...
\begin{tikzpicture}[scale=1.4]
    ...
    % LEFT: Rotated balise with coordinate - THE PROBLEM
    \begin{scope}
        ...
        \begin{scope}[shift={(0,1.3)}, rotate=40]
            ...
            % Coordinate positioned "above" the balise AND ROTATED with it
    ...
    \begin{scope}[xshift=7cm]
        ...
        % Balise now aligned with local axes (horizontal)
...
\end{tikzpicture}
\caption{Prinzip der rotationsinvarianten Koordinatentransformation. Links: Im globalen System ist die Zuordnung zwischen einer rotierten Balise und der zugehörigen Koordinatenangabe geometrisch komplex. Rechts: Nach der Transformation in das lokale System der Balise ist die relative Position trivial bestimmbar ($dy > 0$ bedeutet eindeutig ``oberhalb'').}
...
\subsubsection{Klassenspezifische Suchradien}

Die Größe des Suchbereichs, innerhalb dessen nach zugehörigen Texten gesucht wird, variiert je nach Symbolklasse. Diese Variation ergibt sich aus den unterschiedlichen typischen Abständen zwischen Symbolen und ihren Beschriftungen in technischen Zeichnungen sowie aus den unterschiedlichen Symbol- und Textgrößen. Tabelle \ref{tab:search_radii} zeigt die empirisch ermittelten Suchradien für die wichtigsten Symbolklassen.

\begin{table}[h]
\centering
\begin{tabular}{|l|r|p{6cm}|}
\hline
\textbf{Symbolklasse} & \textbf{Suchradius (px)} & \textbf{Begründung} \\
\hline
Signal & 150 & Signalbezeichnungen können aufgrund von Platzmangel weiter entfernt positioniert sein \\
\hline
Balise & 100 & Balisennummern befinden sich typischerweise direkt am Symbol \\
\hline
Weiche & 200 & Große Symbole mit variabler Textposition, oft mehrere Beschriftungen \\
\hline
Koordinate & 80 & Kilometer-Angaben bilden mit dem Symbol eine kompakte Einheit \\
\hline
Haltepunkt & 120 & Moderate Distanz zur zugehörigen Signalbezeichnung \\
\hline
\end{tabular}
\caption{Klassenspezifische Suchradien für die Symbol-Text-Verknüpfung}
\label{tab:search_radii}
\end{table}

Diese Suchradien wurden durch Analyse von mehreren hundert manuell annotierten Symbol-Text-Paaren ermittelt und stellen einen Kompromiss zwischen Vollständigkeit (alle korrekten Texte werden gefunden) und Präzision (möglichst wenige falsch-positive Kandidaten) dar.

\subsubsection{Richtungsbasierte Filterung}

Neben der Distanzprüfung wird für jede Symbolklasse eine erwartete Richtung definiert, in der sich der zugehörige Text relativ zum Symbol befinden sollte. Dies nutzt domänenspezifisches Wissen über die Konventionen in Gleisplänen. Die Richtungsbedingungen werden im lokalen Koordinatensystem formuliert:

\begin{itemize}
    \item \textbf{Signal}: Text sollte sich \emph{unterhalb} befinden, d.h. $dy_{\text{local}} > 0$. Dies entspricht der üblichen Konvention, dass Signalbezeichnungen unter den Signalsymbolen angeordnet werden.
    
    \item \textbf{Balise}: Text sollte sich \emph{unterhalb oder rechts} befinden, d.h. $dy_{\text{local}} > 0$ oder $dx_{\text{local}} > 0$. Balisennummern sind flexibler positioniert.
    
    \item \textbf{Weiche}: Keine strenge Richtungseinschränkung, da Weichen oft mehrere Beschriftungen in verschiedenen Richtungen aufweisen.
    
    \item \textbf{Koordinate}: Text sollte sich \emph{in der Nähe} befinden, ohne strenge Richtungspräferenz, da Kilometer-Angaben je nach verfügbarem Platz positioniert werden.
\end{itemize}
...
\end{tikzpicture}
\caption{Probabilistische Fenstersuche basierend auf gelernten Offset-Patterns. Ausgehend vom Signal wird die erwartete Textposition (roter Punkt) durch den mittleren gelernten Offset bestimmt. Das Suchfenster (rot gestrichelt) umfasst einen $2\sigma$-Bereich um diese Position. Nur Textkandidaten innerhalb dieses Fensters (grün) werden für die Zuordnung berücksichtigt, während außenliegende Kandidaten (schwarz) ignoriert werden.}
...
Anstatt die Fahrtrichtung als separate Klassifikationsaufgabe zu behandeln, wird sie im implementierten System durch geometrische Analyse der räumlichen Beziehung zwischen Signal und zugeordneter Balise bestimmt. Diese Methode nutzt die in Gleisplänen übliche Konvention, dass die relative Position der Balise zum Signal die Fahrtrichtung kodiert.

Die Fahrtrichtung wird durch die Vorzeichenbetrachtung der lokalen y-Koordinate der Balise relativ zum Signal bestimmt. Hierbei wird die in Abschnitt 6.4.1 beschriebene Koordinatentransformation angewendet, um die Position der Balise im lokalen Koordinatensystem des Signals zu ermitteln. Die Entscheidungsregel lautet:

\begin{equation}
\text{Fahrtrichtung} = \begin{cases} 
\text{A} & \text{falls } dy_{\text{local}}(\text{Balise}, \text{Signal}) < 0 \\
\text{B} & \text{falls } dy_{\text{local}}(\text{Balise}, \text{Signal}) > 0
\end{cases}
\end{equation}

Eine negative lokale y-Koordinate bedeutet, dass die Balise sich oberhalb des Signals in dessen lokaler Orientierung befindet, was nach Konvention der Fahrtrichtung A entspricht. Eine positive lokale y-Koordinate indiziert, dass die Balise unterhalb des Signals liegt, was der Fahrtrichtung B zugeordnet wird.
\begin{figure}[h]
\centering
\begin{tikzpicture}[scale=1.3]
    ...
    \begin{scope}
        ...
        \end{scope}
        \node[green!70!black, font=\small] at (0.8,1.6) {Balise};
        
        \node[font=\footnotesize] at (0,-1.0) {$dy_{\text{local}} < 0$};
        \node[font=\footnotesize] at (0,-1.4) {(Balise oberhalb)};
    \end{scope}
    ...
    \begin{scope}[xshift=5.5cm]
        ...
        \begin{scope}[shift={(0,\yoff-1.4)}]
            ...
            \node[green!70!black, font=\scriptsize] at (0,-0.2) {1234};
        \end{scope}
        \node[green!70!black, font=\small] at (0.8,\yoff-1.6) {Balise};
        
        % Annotation
        \node[font=\footnotesize] at (0,-1.0) {$dy_{\text{local}} > 0$};
        \node[font=\footnotesize] at (0,-1.4) {(Balise unterhalb)};
    \end{scope}
    
\end{tikzpicture}
\caption{Geometrische Bestimmung der Fahrtrichtung eines Signals durch die relative Position der zugeordneten Balise im lokalen Koordinatensystem. Die Koordinatenangabe befindet sich zwischen Signal und Balise. Links: Balise befindet sich oberhalb des Signals ($dy_{\text{local}} < 0$) → Fahrtrichtung A. Rechts: Balise befindet sich unterhalb des Signals ($dy_{\text{local}} > 0$) → Fahrtrichtung B.}
...
Der Gruppierungsalgorithmus iteriert über alle detektierten Haltepunkt-Symbole. Für jedes Haltepunkt-Symbol werden alle Signale und Koordinaten innerhalb eines definierten Umkreises (typischerweise 300 Pixel) als potenzielle Gruppierungspartner betrachtet. Für jede Kombination aus Haltepunkt, Signal und Koordinate wird die Kollinearitätsbedingung geprüft. Die erste Kombination, die die Bedingung erfüllt, wird als gültige Gruppierung akzeptiert und persistiert.
...
\begin{enumerate}
    ...
    \item \textbf{Klassenspezifische Heuristiken}: Unterschiedliche Suchradien und Richtungspräferenzen reflektieren domänenspezifisches Wissen über Zeichnungskonventionen.
...
Die Kombination dieser Techniken erreicht eine Linking-Genauigkeit von über 92\% auf den Testdaten (detaillierte Evaluierung in Kapitel 7), wobei die verbleibenden Fehler hauptsächlich auf ambige Fälle zurückzuführen sind, in denen mehrere Texte ähnliche Distanzen zum Symbol aufweisen.

\section{Datenvalidierung und Qualitätssicherung}
\label{sec:validierungundsicherung}
\label{sec:datenvalidierung}
Die Validierung extrahierter Daten stellt einen kritischen Schritt zur Sicherstellung der Datenqualität in der gesamten Verarbeitungspipeline dar. Während YOLO-basierte Objekterkennung und OCR-Engines bereits intrinsische Konfidenzwerte liefern, erfordert die Anwendung im Kontext von Gleisplanerkennung zusätzliche domänenspezifische Validierungen, die sowohl syntaktische als auch semantische Korrektheit überprüfen. Das implementierte System kombiniert regelbasierte Validierung mit intelligenten Korrekturvorschlägen, um sowohl offensichtliche Fehler automatisch zu beheben als auch komplexere Inkonsistenzen zur manuellen Prüfung zu kennzeichnen.

\subsection{Validierungsarchitektur}

\subsubsection{Grundstruktur des Validierungsframeworks}
Die Validierungsarchitektur basiert auf einem modularen Framework, das die extrahierten Daten in tabellarischer Form entgegennimmt und eine strukturierte Fehleranalyse durchführt. Das zentrale Validierungsmodul analysiert dabei einen DataFrame, der alle Erkennungen aus der YOLO-Pipeline, die zugehörigen OCR-Texte sowie die durch das Linking-Modul hergestellten Verknüpfungen enthält. Die Ausgabe der Validierung besteht aus einer Liste von Validierungsproblemen, wobei jedes Problem durch mehrere Attribute charakterisiert wird.
...
\end{tikzpicture}
\caption{Konfidenzbasierte Entscheidungslogik (Größe skaliert, Schrift angepasst).}
...
\caption{Schematische Darstellung der fünf-Tab Validierungsdialogstruktur}
...
\caption{Bidirektionale Navigation: Jump-to-Detection Workflow zwischen Validierungsdialog und Gleisplan-Ansicht}
...
Die systematische Identifikation und Dokumentation von Änderungen zwischen verschiedenen Versionen eines Gleisplans stellt eine zentrale Anforderung in der Eisenbahnplanung dar. Zur Realisierung dieser Funktionalität wurde ein Diff-Algorithmus implementiert, der auf der Generierung eindeutiger Identifikatoren und mengentheoretischen Operationen basiert.

\subsubsection{Generierung eindeutiger Identifikatoren}

Für die eindeutige Identifikation extrahierter Objekte über verschiedene Planversionen hinweg wird ein deterministischer Unique Identifier (UID) generiert. Die UID-Generierung erfolgt durch Konkatenation der Objektklasse mit dem normalisierten Textinhalt:

\begin{equation}
\text{UID}(o) = \text{class}(o) \oplus \text{``\_''} \oplus \text{normalize}(\text{content}(o))
\end{equation}

wobei $\oplus$ die String-Konkatenation bezeichnet und die Normalisierungsfunktion $\text{normalize}: \Sigma^* \rightarrow \Sigma^*$ Leerzeichen entfernt und Sonderzeichen standardisiert, um konsistente Identifikatoren zu gewährleisten.
\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance=1.5cm,
...
% Example 2: Balise
\node[box, right=2cm of sig1] (bal1) {Balise\\12345};
\node[box, below=0.5cm of bal1] (bal2) {class: ``balise''\\text: ``12345''};
\node[box, below=0.5cm of bal2] (bal3) {\texttt{balise\_12345}};

\draw[arrow] (bal1) -- (bal2);
\draw[arrow] (bal2) -- (bal3);

% Example 3: Coordinate
\node[box, right=2cm of bal1] (coord1) {Koordinate\\18.1606};
...
\begin{itemize}
    ...
    \item Balise mit Kennung ``12345'' $\rightarrow$ \texttt{balise\_12345}
...
\begin{tikzpicture}[scale=1.2]
    ...
    \node[font=\small] at (-1.8, -0.5) {balise\_98765};

    \node[font=\small] at (2, 0.7) {signal\_AS102};
    \node[font=\small] at (2, 0) {coordinate\_18.1606};
    \node[font=\small] at (2, -0.7) {balise\_12345};

    \node[font=\small] at (5.8, 0.5) {signal\_F5};
    \node[font=\small] at (5.8, -0.5) {balise\_11111};
...
balise\_98765 & Entfernt & $x=1500, y=900$ & $-$ \\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{images/Kapitel6/UItracebility.png} 
    \caption{Bidirektionale Navigation zwischen Datenansicht und Quelldokument}
    \label{fig:UItraceability}
\end{figure}
...
Die Verwendung von Connection-Pooling über Context Manager optimiert die Performance bei häufigen Datenbankzugriffen während des Extraktionsprozesses und stellt automatisches Rollback bei Fehlern sicher.

\section{Benutzeroberfläche}
\label{sec:benutzeroberfläche}
Die Benutzeroberfläche dient als interaktive Komponente zur Qualitätskontrolle und manuellen Nachbearbeitung der extrahierten Daten. Obwohl sie nicht zum wissenschaftlichen Kernbeitrag dieser Arbeit gehört, ist sie für die praktische Anwendbarkeit des Systems essentiell. Die Implementierung erfolgte mit einem etablierten GUI Framework für Desktop-Anwendungen und folgt bewährten Designprinzipien der Mensch-Computer-Interaktion.
...
wobei 72 die Standard-DPI-Referenz für PDF-Dokumente darstellt. Der Zoomfaktor $z$ ist im Bereich $[0.25, 4.0]$ steuerbar, was eine 16-fache Vergrößerungsspanne ermöglicht.
\begin{figure}[htbp]
\centering
...
\end{tikzpicture}
\caption{Viewport-basiertes Culling reduziert die Anzahl zu rendernder Overlays auf die im aktuellen Sichtbereich befindlichen Objekte. Dies verbessert die Performance bei großformatigen Plänen erheblich.}
...
\subsubsection{Diff-Report-Export}

Für Versionsvergleiche wird ein dediziertes Excel-Sheet mit den Spalten [\textit{UID, Status, Alte Werte, Neue Werte, Position Alt, Position Neu}] generiert. Dies ermöglicht eine nachvollziehbare Änderungshistorie zwischen Planversionen.
...
\textbf{Objekterkennung (Kapitel 6.2):}
Das YOLOv8-OBB Modell wurde auf einem selbst erstellten Datensatz trainiert und ermöglicht die rotationsinvariante Detektion von Bahnelementen in hochauflösenden PDF-Plänen. Die Inferenz erfolgt mittels einer Tiling-Strategie, die große Dokumente in überlappende Bildkacheln unterteilt und anschließend die Detektionen in das Gesamtkoordinatensystem rücktransformiert.

\textbf{OCR-Pipeline (Kapitel 6.3):}
Die orientierungsadaptive OCR-Pipeline bildet das Herzstück der Textextraktion. Durch die Kaskadierung dreier OCR-Engines (PaddleOCR, Tesseract, EasyOCR) wird eine hohe Erkennungsrobustheit erreicht. Das Dual-Winkel-Routing-System behandelt sowohl achsenparallele als auch beliebig rotierte Texte mittels unterschiedlicher Transformationsstrategien. Klassenspezifische Vorverarbeitungspipelines passen die Bildaufbereitung an die jeweiligen Charakteristika der Symbole an (z.B. adaptive Padding für Signale, morphologische Filterung für Balisenplatten). Die Validierung erfolgt durch regex-basierte Pattern-Matching-Verfahren mit konfidenzgewichteten Scoring-Mechanismen.

\textbf{Symbol-Text-Verknüpfung (Kapitel 6.4):}
Die intelligente Linking-Komponente ordnet extrahierte Texte ihren zugehörigen Symbolen zu. Die rotationsinvariante Koordinatentransformation ermöglicht eine geometrisch konsistente Richtungsbeziehung unabhängig von der Symbolorientierung. Der proximity-basierte Algorithmus nutzt klassenspezifische Suchradien und berücksichtigt erwartete räumliche Relationen. Ein adaptiver Lernmechanismus erfasst wiederkehrende Layoutmuster und optimiert die Verknüpfung durch probabilistische Fenstersuche. Speziallogiken behandeln komplexe Fälle wie die geometrische Ableitung der Fahrtrichtung aus der Balisen-Signal-Relation oder die Gruppierung von Haltepunkt-Tripeln.

\subsection{Unterstützende Systeme}

\textbf{Validierung und Qualitätssicherung (Kapitel 6.5):}
...
\textbf{Versionsvergleich und Rückverfolgbarkeit (Kapitel 6.6):}
...
\textbf{Benutzeroberfläche (Kapitel 6.7):}
...
wobei $N$ die Anzahl der detektierten Objekte bezeichnet und $T_{OCR_i}$ die klassenspezifische OCR-Dauer für Objekt $i$ repräsentiert. Eine detaillierte Performanz-Analyse mit konkreten Zeitmessungen erfolgt in Kapitel 7 (Evaluation).
