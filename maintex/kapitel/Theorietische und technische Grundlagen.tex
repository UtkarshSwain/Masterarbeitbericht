...
Der fundamentale Nachteil von AABB zeigt sich im Verhältnis zwischen der vom Objekt tatsächlich belegten Fläche und der durch die Bounding Box umschlossenen Gesamtfläche. Bei einem um 45° rotierten Textlabel umschließt die achsenparallele Box 41\% irrelevanten Hintergrund, während eine orientierte Box nur 8\% Hintergrund erfasst. Dieser Unterschied ist in Abbildung \ref{fig:aabb_obb_comparison} dargestellt. Die AABB muss alle Ecken des rotierten Objekts umschließen und umfasst dabei große Bereiche außerhalb des eigentlichen Objekts. Die OBB hingegen liegt eng am Objekt an und minimiert den Hintergrundanteil.
...
Der Hintergrundanteil $r_{\text{bg}}$ einer AABB bei einem rotierten rechteckigen Objekt lässt sich geometrisch herleiten. Für ein Objekt mit Seitenverhältnis $\alpha = \frac{w}{h}$ und Rotationswinkel $\theta$ gilt:

\begin{equation}
r_{\text{bg}}(\theta, \alpha) = 1 - \frac{wh}{w_{\text{AABB}} \cdot h_{\text{AABB}}}
\end{equation}

wobei die AABB-Dimensionen gegeben sind durch:

\begin{align}
w_{\text{AABB}} &= w|\cos\theta| + h|\sin\theta| \\
h_{\text{AABB}} &= w|\sin\theta| + h|\cos\theta|
\end{align}

Der Hintergrundanteil erreicht sein Maximum bei $\theta = 45^\circ$. Für ein quadratisches Objekt ($\alpha = 1$) ergibt sich dann $r_{\text{bg}} \approx 50\%$. Die Abhängigkeit des Hintergrundanteils vom Rotationswinkel ist für verschiedene Winkel in Abbildung \ref{fig:rotation_background} visualisiert. Bei 0° Rotation liegt das Objekt perfekt achsenparallel, wodurch sich kein zusätzlicher Hintergrund ergibt. Mit zunehmender Rotation steigt der Hintergrundanteil und erreicht bei 45° sein Maximum von 50\% für quadratische Objekte. Bei 60° nimmt der Hintergrundanteil wieder ab, da sich das Objekt der 90°-Position nähert, bei der es erneut achsenparallel ausgerichtet wäre.

\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.8]
    % Four rotation examples: 0°, 30°, 45°, 60°
    \foreach \angle/\xpos/\bg in {0/0/0, 30/3.5/29, 45/7/50, 60/10.5/29} {
        \begin{scope}[shift={(\xpos,0)}]
            % Object (rotated)
            \begin{scope}[rotate=\angle]
                \draw[thick, fill=blue!20] (-0.5,-0.3) rectangle (0.5,0.3);
            \end{scope}
            
            % AABB
            \pgfmathsetmacro{\w}{abs(cos(\angle))*0.5 + abs(sin(\angle))*0.3}
            \pgfmathsetmacro{\h}{abs(sin(\angle))*0.5 + abs(cos(\angle))*0.3}
            \draw[thick, red, dashed] (-\w,-\h) rectangle (\w,\h);
            
            % Labels
            \node[below, font=\footnotesize] at (0, -1.3) {$\theta = \angle^\circ$};
            \node[below, font=\footnotesize, red] at (0, -1.7) {bg: \bg\%};
        \end{scope}
    }
\end{tikzpicture}
\caption{Hintergrundanteil bei verschiedenen Rotationswinkeln.}
\label{fig:rotation_background}
\end{figure}

Dieser hohe Hintergrundanteil hat zwei wesentliche Konsequenzen für die Objekterkennung: Erstens verschlechtert sich das Signal-Rausch-Verhältnis der Eingabedaten für nachgelagerte Verarbeitungsschritte wie OCR. Zweitens steigt die Wahrscheinlichkeit falscher Detektionen, da irrelevante Bildelemente innerhalb der Bounding Box liegen.

\subsubsection{Non-Maximum Suppression bei dichten Objektgruppen}

Ein weiteres Problem ergibt sich bei der Nachbearbeitung von Detektionen mittels Non-Maximum Suppression (NMS). NMS eliminiert redundante Detektionen durch Unterdrückung überlappender Boxen basierend auf ihrer Intersection over Union (IoU):

\begin{equation}
\text{IoU}(B_1, B_2) = \frac{\text{Area}(B_1 \cap B_2)}{\text{Area}(B_1 \cup B_2)}
\end{equation}

Bei dicht platzierten rotierten Objekten führen AABB zu hohen IoU-Werten, obwohl die Objekte selbst nicht überlappen. Dies resultiert in fälschlicher Unterdrückung korrekter Detektionen. Abbildung \ref{fig:nms_problem} demonstriert dieses Fehlverhalten anhand zweier nahe beieinanderliegender Textlabels. Die beiden Textobjekte sind um 30° bzw. -30° rotiert und überlappen sich nicht. Ihre achsenparallelen Bounding Boxes hingegen weisen eine signifikante Überlappung mit IoU = 0.42 auf. Bei einem typischen NMS-Schwellenwert von $\tau = 0.4$ würde eine der beiden korrekten Detektionen fälschlicherweise unterdrückt. Im Gegensatz dazu berechnet sich die IoU zwischen den orientierten Bounding Boxes korrekt zu 0.0, da die Objekte tatsächlich nicht überlappen.
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.9]
    ...
    \begin{scope}[rotate around={30:(1.5,2)}]
        % ÄNDERUNG HIER: Randfarbe auf Grün (green!70!black) und 'very thick' gesetzt
        % Dies repräsentiert die OBB
...
Der Rotationswinkel $\theta$ weist eine zyklische Periodizität auf: Eine Rotation um 180° (bzw. bei symmetrischen Objekten um 90°) resultiert in einer geometrisch identischen Bounding Box. Dies führt zu Ambiguitäten in der Netzwerk-Ausgabe. Betrachtet man beispielsweise zwei Vorhersagen $\theta_1 = 0^\circ$ und $\theta_2 = 89^\circ$ für dasselbe Objekt, so sind diese geometrisch nur 1° voneinander entfernt, numerisch jedoch 89 Einheiten. Standard-Regressionsverluste (z.B. Smooth L1) behandeln diese Diskrepanz linear, was zu instabilen Gradienten führt.
...
Die Box-Loss $\mathcal{L}_{\text{box}}$ quantifiziert die Lokalisierungsgenauigkeit der vorhergesagten OBB gegenüber der Ground Truth. Die Classification-Loss $\mathcal{L}_{\text{cls}}$ verwendet Binary Cross-Entropy für die Klassenzuordnung\cite{Lin2017_FocalLoss}. Die Distribution Focal Loss (DFL) verbessert die Bounding-Box-Regression durch Modellierung der Koordinaten als Verteilungen anstelle von Punktschätzungen.
