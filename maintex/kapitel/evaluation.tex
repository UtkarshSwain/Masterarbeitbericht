...
\hspace{3mm} GKS (beide Arten) & 31 \\
\hspace{3mm} GM-Blöcke & 27 \\
\hspace{3mm} \textbf{Summe (12 Klassen)} & \textbf{164} \\
...
Die Detektionsleistung variiert zwischen den verschiedenen Symbolklassen moderat. Tabelle~\ref{tab:detection_per_class} zeigt die klassenspezifischen Metriken für alle 13~Klassen. Precision und Recall wurden basierend auf den Werten der Konfusionsmatrix (Abbildung~\ref{fig:confusion_matrix_eval}) berechnet, während die mAP@0.5-Werte die durchschnittliche Precision über alle Recall-Stufen repräsentieren.
...
weichen\_block & 129 & --- & --- & 97.7\% \\
\hline
\textbf{Alle Klassen (Durchschnitt)} & \textbf{3.076} & \textbf{---} & \textbf{---} & \textbf{98.4\%} \\
\hline
\end{tabular}
\caption{Detektionsmetriken für alle 13~Klassen auf dem Validierungsdatensatz. mAP@0.5 aus den Precision-Recall-Kurven.}
...
\textbf{FA-003 (Zielobjekte):} Alle 13~Symbolklassen werden zuverlässig detektiert. Die Klasse \textit{coordinate} liefert Positionsinformationen, die den anderen 12~Klassen zugeordnet werden.
\begin{figure}[H]
\centering
...
Die Matrix zeigt, dass 5 Instanzen von \textit{gks\_gesteuert} 
...
\begin{itemize}
    ...
    \item \textbf{Herausforderung bei GKS-Varianten}: Die Klasse \textit{gks\_gesteuert} zeigt mit 94.5\% die niedrigste Precision, was auf die hohe visuelle Ähnlichkeit zu \textit{gks\_festkodiert} zurückzuführen ist. Die Konfusionsmatrix (Abbildung~\ref{fig:confusion_matrix_eval}) zeigt, dass 4 Instanzen von \textit{gks\_gesteuert} fälschlicherweise als \textit{gks\_festkodiert} klassifiziert wurden, während 1 Instanz von \textit{gks\_festkodiert} als \textit{gks\_gesteuert} fehlklassifiziert wurde. Diese beiden GKS-Arten unterscheiden sich nur durch interne Symboldetails (fest kodierte vs. programmierbarer Code), was die Unterscheidung für das neuronale Netz erschwert.
...
Die Gesamtsystemleistung wird durch die End-to-End Accuracy gemessen, die alle Pipeline-Stufen integriert und der in Anforderung \textbf{NFA-003} definierten Zielmetrik entspricht. Die Evaluation umfasst alle 12~Symbolklassen, die mit Koordinaten verknüpft werden.
...
\hline
\end{tabular}
\caption{Attribut-spezifische Genauigkeit für Signale (9 Testpläne). Ein Signal gilt als 
...
Plan 7 & 40/40 & 55/55 & 46/46 & 141/141 & Keine Fehler (perfekt) \\
 & (100\%) & (100\%) & (100\%) & (100\%) & (\enquote{w}/\enquote{W} irrelevant) \\
\hline
\textbf{Gesamt} & \textbf{250/256} & \textbf{272/278} & \textbf{241/247} & \textbf{1432/1473} & \textbf{41 Fehler} \\
 & \textbf{(98.26\%)} & \textbf{(99.22\%)} & \textbf{(98.61\%)} & \textbf{(97.22\%)} & \\
\hline
\end{tabular}
...
\textbf{Anmerkung:} Die dargestellten Ergebnisse basieren auf neun unabhängigen
Testplänen.
...
\hline
Plan 3 & 1 & weichen\_block & O/0-Verwechslung in Koordinaten (2 Instanzen) \\
\hline
...
Nachbarrauschen (extra Zeichen erfasst) & 8 & 19.5\% \\
Fehlende Ziffern (Bbox-bedingt) & 7 & 17.1\% \\
OCR komplett fehlgeschlagen & 5 & 12.2\% \\
...
\begin{enumerate}
    ...
    \item \textbf{Nachbarrauschen (8 Fehler, 19.5\%):}
    ...
    \item \textbf{Fehlende Ziffern durch Bbox-Begrenzung (7 Fehler, 17.1\%):}
    ...
    \item \textbf{Falsche Koordinatenzuordnung (8 Fehler, 19.5\%):}
    Diese Linking-Fehler umfassen falsch zugeordnete Koordinaten (z.B. \textit{weichengruppenende} mit falscher Koordinate) sowie vertauschte Zuordnungen zwischen \textit{gks} und \textit{isolierstoß}. Der Proximity-basierte Algorithmus wählte in diesen Fällen die falsche Kandidaten-Koordinate. Verbesserungen durch kontextbasiertes Ranking oder maschinelles Lernen der Layoutpräferenzen sind möglich.

    \item \textbf{OCR komplett fehlgeschlagen (5 Fehler, 12.2\%):}
...
\begin{itemize}
    \item Die E2E-Genauigkeit von 97.22\% auf dem Testsatz übertrifft die Anforderung \textbf{NFA-003} ($\geq$ 85\%) deutlich
...
Um den verbleibenden manuellen Korrekturaufwand (geschätzt 5-10\% der Objekte bei produktivem Einsatz) zu minimieren und die Qualitätssicherung zu erleichtern, wurden umfangreiche Validierungs- und Korrekturwerkzeuge in die Benutzeroberfläche integriert. Diese erfüllen die Anforderungen \textbf{FA-008} (Manuelle Korrektur), \textbf{FA-012} (Visuelle Validierung) und \textbf{NFA-005} (Prüfbarkeit).
...
\begin{itemize}
    ...
    \item \textbf{Änderungsverfolgung}: Alle manuellen Korrekturen werden protokolliert (Anforderung FA-011)
\end{itemize}
...
\textbf{NFA-004 (Robustheit):} Alle 9 Testpläne wurden ohne Systemabstürze oder 
...
\hline
\hline
\textbf{Gesamt (9 Pläne)} & \textbf{92 Ø} & \textbf{102 Ø} & \textbf{12.3 min (739s)} & \textbf{8.4s} \\
\hline
\end{tabular}
...
Der KI-gestützte Prozess reduziert den Gesamtzeitaufwand um \textbf{75,3\%}, von durchschnittlich 64 Minuten auf 15,8 Minuten pro Plan (\textbf{NFA-006}). Bei typischen Siemens Mobility Projekten mit 20-50 Gleisplänen ergibt sich eine Gesamtzeitersparnis von 16-40 Stunden (2-5 Arbeitstage) pro Projekt.

\textbf{Anmerkung zur Messgenauigkeit:} Die manuelle Zeitmessung erfolgte an einem fokussierten Planabschnitt (25,3s/Objekt), hochgerechnet auf 32s/Objekt für vollständige A0-Pläne unter Berücksichtigung des zusätzlichen Aufwands für räumlich verteilte Symbole.

\textbf{Skalierbarkeit:} Die gemessenen Zeiten basieren auf CPU-only Verarbeitung (\textbf{NFA-001}). Die durchschnittliche Verarbeitungszeit von 12,3 Minuten pro A0-Plan auf Standard-CPU-Hardware erfüllt die Anforderung \textbf{NFA-007}. Eine optionale GPU-Beschleunigung würde die Gesamtzeit auf 2-3 Minuten pro Plan reduzieren (ca.\ 4-6× schneller).
...
\begin{itemize}
    ...
    \item \textbf{FA-003} (Zielobjekte): Alle 13~Symbolklassen erfolgreich detektiert 
    (Tabelle~\ref{tab:detection_per_class})
    \item \textbf{FA-004} (OCR-Genauigkeit): Integriert in E2E-Genauigkeit von 97.22\% 
    ...
    \item \textbf{FA-007} (Symbol-Koordinaten-Verknüpfung): 99.12\% Linking-Genauigkeit 
...
\hline
\multicolumn{3}{|c|}{\textit{Datenaufbereitung und Export (FA-009 bis FA-011)}} \\
\hline
...
\hline
\multicolumn{3}{|c|}{\textit{Benutzerinteraktion und Konfiguration (FA-012 bis FA-014)}} \\
\hline
...
\hline
FA-014 & Modulare Architektur: Alle 13~Symbolklassen erfolgreich integriert; modulare Architektur (§\ref{chap:konzeption}) ermöglicht unabhängige Weiterentwicklung der Komponenten & Bestanden \\
\hline
...
\textbf{FA-014 (Modulare Architektur):} Die modulare Architektur ermöglicht die Integration aller 13~Symbolklassen. Die Klassen wurden modular konfiguriert mit klassenspezifischen Parametern für OCR-Pipeline, Linking-Algorithmus und Export-Modul. Alle Klassen erreichen hohe Detektionsleistungen (Ø mAP@0.5: 98.4\%).

\textbf{NFA-008 (Update-Fähigkeit):} Die trainierten Modellgewichte werden als externe
...
\textbf{NFA-009 (Eingabeformate):} Alle 9 Testpläne wurden als PDF-Dateien
...
\textbf{NFA-010 (Ausgabeformate):} Das System unterstützt alle geforderten 
...
Die Anforderungen FA-009 bis FA-011 betreffen unterstützende Funktionen, deren
...
FA-004 & OCR-Genauigkeit & \checkmark (integriert in 97.22\% E2E) \\
FA-005 & OCR-Robustheit & \checkmark (100\% bei $|\theta|>30^\circ$) \\
FA-006 & Fahrtrichtungsdetektion & \checkmark (99.42\%) \\
FA-007 & Symbol-Koordinaten-Verknüpfung & \checkmark (99.12\%) \\
...
NFA-003 & Gesamtsystem-Genauigkeit $\geq$ 85\% & \checkmark (97.22\%) \\
NFA-004 & Robustheit & \checkmark \\
NFA-005 & Prüfbarkeit & \checkmark \\
NFA-006 & Prozessoptimierung & \checkmark (75\% Zeitersparnis) \\
NFA-007 & Ressourceneffizienz & \checkmark (12.3 min/Plan) \\
NFA-008 & Update-Fähigkeit & \checkmark \\
NFA-009 & Eingabeformate & \checkmark \\
NFA-010 & Ausgabeformate & \checkmark \\
\hline
\end{tabular}
...
\begin{itemize}
    \item Gesamtgenauigkeit von \textbf{97.22\%} auf dem Testsatz übertrifft Zielwert von 85\% (NFA-003) deutlich
