...
\hline
Anzahl Original-Gleispläne (A0-Format) & 24 \\
Anzahl Tiles (2048x2048, überlappend) & 1.131 \\
\hspace{3mm} davon Trainingsdaten & 923 \\
\hspace{3mm} davon Validierungsdaten & 208 \\
Annotierte Symbole (Validierungsset) & 3.076 \\
\hline
\multicolumn{2}{|l|}{\textit{Kernklassen (produktionsrelevant)}} \\
\hline
\hspace{3mm} Signale & 473 \\
\hspace{3mm} Koordinaten & 1.483 \\
\hspace{3mm} GKS (festkodiert) & 122  \\
\hspace{3mm} GKS (gesteuert) & 157  \\
\hspace{3mm} GM-Blöcke & 201  \\
\hspace{3mm} \textbf{Summe Kernklassen} & \textbf{2.436 (79,2\%)} \\
\hline
\multicolumn{2}{|l|}{\textit{Auxiliarklassen (experimentell)}} \\
\hline
\hspace{3mm} Sonstige Klassen (8 Typen) & 640 (20.8\%) \\
\hline
...
\textbf{Anmerkung zur Datenaufteilung:} Die Aufteilung in Trainings- und 
Validierungsdaten erfolgte stratifiziert mit einem 80/20-Verhältnis 
(vgl. Abschnitt~\ref{subsubsec:dataset_split}). Sowohl Original- als auch 
augmentierte Tiles sind in beiden Teilmengen enthalten, wobei die relative 
Klassenverteilung annähernd erhalten bleibt.

\textbf{Verwendungszweck:} Der Validierungssatz dient der technischen Bewertung 
der YOLO-Detektionsleistung (mAP, Precision, Recall) und bestätigt die erfolgreiche 
Modellkonvergenz. Die detaillierten Trainingsergebnisse und Konvergenzanalysen 
sind in Abschnitt~\ref{subsec:modelltraining} dokumentiert. Der Validierungssatz 
stellt keine vollständig unabhängige Testmenge dar; die unabhängige End-to-End-Evaluation 
erfolgt auf dem separaten Testsatz (Abschnitt~\ref{subsec:e2e_test_eval}).
...
\textbf{Hinweis zu weiteren Symbolklassen:} Das Modell wurde zusätzlich auf acht weitere Klassen trainiert (\textit{weichen\_block}, \textit{haltepunkt}, \textit{isolierstoß}, \textit{sverbinder}, \textit{prellbock}, \textit{haltetafel}, \textit{endeweichen}, \textit{weichengruppeende}), die zu experimentellen Zwecken annotiert wurden. Diese Klassen erreichen ebenfalls hohe Erkennungsraten (Durchschnitt mAP@0.5: 98.8\%), werden jedoch in der nachfolgenden End-to-End-Evaluation nicht berücksichtigt, da sie nicht zur primären Extraktionsaufgabe gehören.
\begin{figure}[H]
\centering
