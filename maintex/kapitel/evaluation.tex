...
\textbf{Methodische Einschränkung:} Aufgrund des erheblichen manuellen Aufwands zur Erstellung vollständiger Ground-Truth-Daten für A0-Gleispläne (durchschnittlich 2-3 Stunden pro Plan) sowie der begrenzten Verfügbarkeit weiterer ungesehener Pläne wurden fünf Pläne unterschiedlicher Komplexität für die End-to-End-Evaluation ausgewählt. Diese Pläne repräsentieren verschiedene Komplexitätsstufen und ermöglichen eine realistische Bewertung der Systemleistung. Für eine vollständig unabhängige Evaluation wären zusätzliche, komplett ungesehene Pläne wünschenswert gewesen, was im Zeitrahmen dieser Masterarbeit jedoch nicht realisierbar war.
...
Komplexitätsstufen & Einfach (1), Mittel (2), Komplex (2) \\
\hline
\multicolumn{2}{|l|}{\textit{Durchschnittliche Symbolanzahl pro Plan}} \\
\hline
\hspace{3mm} Signale & 23 \\
\hspace{3mm} GKS-Platten (beide Typen) & 36 \\
\hspace{3mm} GM-Blöcke & 31 \\
\hspace{3mm} \textbf{Summe Kernklassen} & \textbf{90} \\
...
\hline
Einfach & Niedrige Symboldichte ($<$ 60 Symbole), klare räumliche Trennung, typisch für Streckenabschnitte & 1 \\
\hline
...
\hline
signal (Name + Position + Richtung) & 116 & 114 & 98.28\% \\
gks (Nummer + Position, beide Typen) & 178 & 177 & 99.44\% \\
gm\_block (Position) & 154 & 151 & 98.05\% \\
\hline
\textbf{Durchschnitt (Kernklassen)} & \textbf{448} & \textbf{442} & \textbf{98.66\%} \\
\hline
\end{tabular}
\caption{End-to-End Genauigkeit pro Objektklasse (5 Testpläne)}
...
\hline
Vollständig korrekt extrahiert (Kernklassen) & $\geq$ 85\% & 98.66\% & \checkmark \\
Manuelle Korrektur erforderlich & $\leq$ 15\% & 1.34\% & \checkmark \\
\hline
\end{tabular}
...
Das System erreicht eine End-to-End-Genauigkeit von \textbf{98.66\%} für die Kernklassen auf dem Testsatz, was die Anforderung NFA-003 ($\geq$ 85\%) deutlich übertrifft. Dies bedeutet, dass bei 98.66\% aller extrahierten Objekte Name/Nummer, Position und (bei Signalen) Fahrtrichtung vollständig korrekt extrahiert wurden. Nur 1.34\% der Objekte erfordern manuelle Nachbearbeitung.

\textbf{Hinweis zur Generalisierung:} Die evaluierten fünf Testpläne repräsentieren eine erste Stichprobe des Leistungsspektrums. Bei umfangreicheren Tests auf diverseren Plänen mit variierenden Qualitätsmerkmalen (z.B. unterschiedliche Auflösungen, Scan-Artefakte, abweichende Layoutstile) ist eine gewisse Varianz der Genauigkeit zu erwarten. Basierend auf der beobachteten Fehlerverteilung und unter Berücksichtigung potenzieller Herausforderungen bei komplexeren oder qualitativ schlechteren Eingabedaten wird eine realistische End-to-End-Genauigkeit im Bereich von \textbf{90-95\%} für den produktiven Einsatz erwartet. Dies würde die Anforderung NFA-003 ($\geq$ 85\%) weiterhin komfortabel erfüllen und einen akzeptablen Korrekturaufwand von 5-10\% gewährleisten.
...
\begin{tabular}{|l|r|r|r|r|}
\hline
\textbf{Plan} & \textbf{Signale} & \textbf{GKS} & \textbf{GM} & \textbf{Gesamt} \\
\hline
Plan 1 & 16/16 (100\%) & 21/21 (100\%) & 20/21 (95.2\%) & 57/58 (98.3\%) \\
Plan 2 & 12/12 (100\%) & 30/30 (100\%) & 27/28 (96.4\%) & 69/70 (98.6\%) \\
Plan 3 & 40/40 (100\%) & 54/54 (100\%) & 45/46 (97.8\%) & 139/140 (99.3\%) \\
Plan 4 & 21/21 (100\%) & 30/30 (100\%) & 23/23 (100\%) & 74/74 (100\%) \\
Plan 5 & 25/27 (92.6\%) & 42/43 (97.7\%) & 36/36 (100\%) & 103/106 (97.2\%) \\
\hline
\textbf{Gesamt} & \textbf{114/116 (98.3\%)} & \textbf{177/178 (99.4\%)} & \textbf{151/154 (98.1\%)} & \textbf{442/448 (98.7\%)} \\
\hline
\end{tabular}
\caption{Detaillierte End-to-End Ergebnisse pro Testplan (Kernklassen)}
\label{tab:e2e_per_plan_test}
\end{table}

\textbf{Fehlerquellenanalyse:}

Um die Optimierungspotenziale zu identifizieren, wurden die 6 aufgetretenen Fehler nach Fehlerquelle analysiert:
...
\hline
OCR-Fehler (Koordinate nicht gelesen) & 3 & 50.0\% \\
OCR-Fehler (Signal-Koordinate fehlerhaft) & 2 & 33.3\% \\
OCR-Fehler (GKS-Koordinate fehlerhaft) & 1 & 16.7\% \\
YOLO Detection-Fehler & 0 & 0.0\% \\
YOLO Classification-Fehler & 0 & 0.0\% \\
Linking-Fehler & 0 & 0.0\% \\
\hline
\textbf{Gesamt} & \textbf{6} & \textbf{100\%} \\
\hline
\end{tabular}
...
Die Fehleranalyse zeigt, dass \textbf{alle 6 Fehler} auf OCR-Probleme zurückzuführen sind, während YOLO-Detektion, Klassifikation und Linking fehlerfrei funktionierten. Von den 6 OCR-Fehlern:
\begin{itemize}
    \item \textbf{3 Fehler (50\%)}: GM-Block-Koordinaten wurden von OCR nicht gelesen (Plan 1, 2, 3 -- jeweils 1 GM-Koordinate)
    \item \textbf{2 Fehler (33\%)}: Signal-Koordinaten wurden nicht extrahiert (Plan 5 -- 2 Signale ohne Koordinate)
    \item \textbf{1 Fehler (17\%)}: GKS-Koordinate wurde falsch erkannt (Plan 5 -- 1 GKS mit falscher Koordinate)
\end{itemize}

Diese Verteilung zeigt klare Optimierungsansätze: Die OCR-Komponente, insbesondere für numerische Koordinaten, ist der primäre Verbesserungshebel. Die 100\% Erfolgsrate der YOLO-Komponente und des Linking-Moduls bestätigen deren Produktionsreife.
...
Komplex (Plan 4, 5) & 2 & 180 & 98.33\% & 3 (2× Signal, 1× GKS) \\
\hline
...
\textbf{Typische Fehlerquellen (OCR-bezogen):}
\begin{enumerate}
    \item \textbf{GM-Block-Koordinaten}: 3 von 154 GM-Positionen (1.95\%) wurden nicht gelesen. Vermutlich aufgrund niedriger Auflösung oder ungünstiger Rotation der Koordinatenbeschriftung
    \item \textbf{Signal-Koordinaten}: 2 von 116 Signalen (1.72\%) hatten keine extrahierte Koordinate. Dies betraf ausschließlich Plan 5, möglicherweise aufgrund dichter Überlappung
    \item \textbf{GKS-Koordinaten}: 1 von 178 GKS-Platten (0.56\%) hatte eine fehlerhafte Koordinate, vermutlich O/0 oder 1/I Verwechslung
\end{enumerate}
...
\begin{itemize}
    ...
    \item Nur 6 von 448 Objekten (1.34\%) im Testsatz erforderten Korrektur; im produktiven Einsatz werden 5-10\% erwartet
