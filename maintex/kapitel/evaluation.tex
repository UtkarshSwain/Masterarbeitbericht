...
eines um $37.5^\circ$ rotierten Koordinatentexts demonstriert die erfolgreiche Verarbeitung 
...
\begin{itemize}
    ...
    bei $\theta = 37{,}5^\circ$ (§\ref{sec:ocrpipeline})
...
Die interne Verarbeitungspipeline konvertiert PDFs zunächst in hochauflösende 
Rasterbilder (500 DPI, PNG-Format) mittels PyMuPDF (§6.2.3), bevor die 
YOLO-Inferenz erfolgt. Damit wird die Bildverarbeitungsfähigkeit (PNG/JPG) 
implizit durch jeden PDF-Test validiert. Ein direkter Import von Bilddateien 
ohne PDF-Konvertierung wurde nicht explizit getestet, ist jedoch aufgrund 
der identischen nachgelagerten Pipeline-Stufen funktional äquivalent.
