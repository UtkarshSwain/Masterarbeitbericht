...
Der Datensatz umfasst 208 Validierungs-Tiles mit 3.076 annotierten Symbolinstanzen, davon 2.436 Instanzen (79,2\%) in den fünf Kernklassen (\textit{signal}, \textit{coordinate}, \textit{gks\_festkodiert}, \textit{gks\_gesteuert}, \textit{gm\_block}) und 640 Instanzen (20,8\%) in acht Auxiliarklassen (vgl.\ Tabelle~\ref{tab:dataset_split} und Tabelle~\ref{tab:dataset_distribution}).
...
\hline
\textbf{Kernklassen (Durchschnitt)} & \textbf{2.436 (79,2\%)} & \textbf{98.3\%} & \textbf{97.7\%} & \textbf{97.7\%} \\
\hline
\end{tabular}
...
von 8 Auxiliarklassen demonstriert die Erweiterbarkeit des Prototyps (NFA-008).

\textbf{Hinweis zu weiteren Symbolklassen:} Das Modell wurde zusätzlich auf acht weitere Klassen trainiert (\textit{weichen\_block}, \textit{haltepunkt}, \textit{isolierstoß}, \textit{sverbinder}, \textit{prellbock}, \textit{haltetafel}, \textit{endeweichen}, \textit{weichengruppenende}), die zu experimentellen Zwecken annotiert wurden. Diese Klassen erreichen ebenfalls hohe Erkennungsraten (Durchschnitt mAP@0.5: 98.8\%), werden jedoch in der nachfolgenden End-to-End-Evaluation nicht berücksichtigt, da sie nicht zur primären Extraktionsaufgabe gehören.
\begin{figure}[H]
\centering
...
\begin{itemize}
    ...
    \item \textbf{Perfekt balancierte Klassen}: Die Klassen \textit{gm\_block} (97.6\% P/R) und \textit{coordinate} (97.7\% P, 98.6\% R) zeigen nahezu identische Precision- und Recall-Werte, was auf eine optimale Detektionsqualität ohne systematische Bias hinweist. Bei \textit{coordinate} ist dies besonders bemerkenswert, da diese Klasse mit 1.483 Instanzen (60,9\% der Kernklassen) die am häufigsten vorkommende ist.
    
    \item \textbf{Herausforderung bei GKS-Varianten}: Die Klasse \textit{gks\_gesteuert} zeigt mit 94.5\% die niedrigste Precision unter den Kernklassen, was auf die hohe visuelle Ähnlichkeit zu \textit{gks\_festkodiert} zurückzuführen ist. Die Konfusionsmatrix (Abbildung~\ref{fig:confusion_matrix_eval}) zeigt, dass 4 Instanzen von \textit{gks\_gesteuert} fälschlicherweise als \textit{gks\_festkodiert} klassifiziert wurden, während 1 Instanz von \textit{gks\_festkodiert} als \textit{gks\_gesteuert} fehlklassifiziert wurde. Diese beiden GKS-Arten unterscheiden sich nur durch interne Symboldetails (fest kodierte vs. programmierbarer Code), was die Unterscheidung für das neuronale Netz erschwert.
...
\textbf{FA-014 (Modulare Architektur):} Die Erweiterbarkeit der modularen Architektur wurde durch die Integration
...
\textbf{NFA-009 (Eingabeformate):} Alle 7 Testpläne wurden als PDF-Dateien
