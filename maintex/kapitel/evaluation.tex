...
\hspace{3mm} GKS (beide Arten) & 37 \\
\hspace{3mm} GM-Blöcke & 31 \\
\hspace{3mm} \textbf{Summe (12 Klassen)} & \textbf{92} \\
...
\hline
\textbf{Durchschnitt (alle Klassen)} & \textbf{644} & \textbf{636} & \textbf{98.76\%} \\
\hline
\end{tabular}
...
\hline
Vollständig korrekt extrahiert (alle Klassen) & $\geq$ 85\% & 98.76\% & \checkmark \\
...
\textbf{Gesamt} & \textbf{169/172} & \textbf{254/256} & \textbf{213/216} & \textbf{636/644} & \textbf{8 Fehler} \\
 & \textbf{(98.26\%)} & \textbf{(99.22\%)} & \textbf{(98.61\%)} & \textbf{(98.76\%)} & \\
\hline
\end{tabular}
...
\hline
\end{tabular}
\caption{Detaillierte Fehleranalyse mit Root Causes (9 Testpläne)}
...
\begin{enumerate}
    \item \textbf{YOLO Bounding Box Regression (2 Fehler, 25.0\%):} 
    ...
    \item \textbf{OCR-Extraktion fehlgeschlagen (2 Fehler, 25.0\%):} 
    ...
    \item \textbf{OCR-Fehlinterpretation durch visuelle Artefakte (1 Fehler, 12.5\%):} 
    ...
    \item \textbf{Linking bei atypischem Layout (1 Fehler, 12.5\%):} 
    ...
    \item \textbf{Fahrtrichtungs-Ableitung fehlgeschlagen (1 Fehler, 12.5\%):}
    ...
    \item \textbf{Ein Classification-Fehler bei GKS-Typen (1 Fehler, 12.5\%):} 
        ...
        Konfusionsmatrix (Abbildung~\ref{fig:confusion_matrix_eval}) beobachtete 
        Verwechslungstendenz zwischen diesen visuell ähnlichen Symboltypen. 
        Die beiden GKS Arten unterscheiden sich nur durch interne 
...
\hline
\textbf{Gesamt} & \textbf{9 Pläne} & \textbf{644} & \textbf{98.76\%} & \textbf{8} \\
\hline
\end{tabular}
