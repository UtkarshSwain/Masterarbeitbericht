...
\textbf{Validierung FA-001 und FA-002:}
...
synthetischen Rotationsaugmentation (10 Winkel von $-90°$ bis $+90°$, vgl. 
§\ref{subsec:Modelltraining}) gewährleistet robuste Erkennung bei beliebigen 
Symbolorientierungen. Die konstante Detektionsleistung über alle 7 Testpläne -- 
die Symbole in diversen Orientierungen enthalten -- bestätigt die Rotationsinvarianz 
empirisch. Eine dedizierte Winkel-Stratifizierung wurde nicht durchgeführt, da die 
Ground-Truth-Annotationen keine explizite Winkelinformation enthalten.
...
Die Bewertung des Systems erfolgt auf mehreren Ebenen mit jeweils spezifischen Metriken. Die verwendeten Metriken basieren auf den in Kapitel~\ref{chap:theoretischeundtechnischegrundlagen} eingeführten Standardverfahren für Objekterkennung (Abschnitt~\ref{subsec:evaluationsmetriken}) und OCR-Systeme (Abschnitt~\ref{subsec:ocr_metriken}). Dieser Abschnitt fasst die angewandten Metriken kurz zusammen und definiert die spezifische End-to-End Systemmetrik.
...
\textbf{FA-005 (OCR-Robustheit):} Die Multi-Engine-Kaskade (PaddleOCR → Tesseract → 
EasyOCR) mit Dual-Winkel-Routing gewährleistet robuste Texterkennung auch unter 
erschwerten Bedingungen. Das in Abb.~\ref{fig:angular_ocr_example} gezeigte Beispiel 
eines um 37.5° rotierten Koordinatentexts demonstriert die erfolgreiche Verarbeitung 
nicht-kardinaler Orientierungen. Die klassenspezifische Vorverarbeitung (morphologische 
Filterung für GKS, Multi-Scale für Koordinaten) adressiert domänenspezifische 
Herausforderungen wie Linien-Artefakte und niedrige Auflösung.
...
(vgl. Anhang~\ref{app:all_classes}).
...
\textbf{NFA-010 (Eingabeformate):} Alle 7 Testpläne wurden als PDF-Dateien verarbeitet. 
Die Rasterisierung erfolgte mit PyMuPDF bei 500 DPI. Die Pipeline unterstützt zusätzlich 
PNG- und JPG-Eingaben, was jedoch im Rahmen dieser Evaluation nicht explizit getestet wurde.
...
\textbf{NFA-012 (Ausgabeformate):} Der Export in XLSX-Format wurde für alle 7 Testpläne 
durchgeführt (vgl. §\ref{subsec:export_validation}). Die Export-Optionen für CSV und 
JSON sind implementiert und wurden durch Funktionstests validiert 
(Tab.~\ref{tab:functional_validation}).
...
\begin{itemize}
    ...
    \item Robuste Rotationsinvarianz durch OBB-Architektur und synthetische Augmentation
...
\begin{itemize}
    ...
    \item Fehlerverteilung: 3 YOLO (37.5\%), 3 OCR (37.5\%), 2 Linking (25.0\%)
    \item Stabile Leistung über alle Komplexitätsstufen (98.23\% -- 99.31\%)
    \item Produktionsreif für den Einsatz bei Siemens Mobility
\end{itemize}
