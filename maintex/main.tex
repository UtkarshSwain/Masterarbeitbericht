...
\usetikzlibrary{decorations.pathreplacing, arrows.meta, positioning, shadows,shapes, ,shapes.geometric, calc, fit}
\usepackage{placeins}
\usepackage{pgfplots}
...
% --- ANGEPASSTES FANCY HEADER SETUP (für twoside) ---
\pagestyle{fancy}
\fancyhf{} % Clear all header and footer fields
\fancyhead[LO,RE]{% Innerer Header (Links auf ungeraden, Rechts auf geraden)
    \includegraphics[height=1cm]{images/tu-clausthal-logo.jpg}\\ % Logo (adjust size and path)
    \nouppercase{\leftmark} % Chapter name below the logo
}
\fancyhead[LE,RO]{\thepage} % Äußerer Header: Seitenzahl
\renewcommand{\headrulewidth}{0.4pt} % Line below the header

% Start header only from the Introduction chapter
\fancypagestyle{plain}{
    \fancyhf{} % Clear existing header/footer
    \fancyhead[LO,RE]{% Innerer Header
        \includegraphics[height=1cm]{images/tu-clausthal-logo.jpg}\\% Logo
        \nouppercase{\leftmark} % Chapter name below logo
    }
    \fancyhead[LE,RO]{\thepage} % Äußerer Header: Page number
    \renewcommand{\headrulewidth}{0.4pt} % Header line
}
% --- ENDE ANGEPASSTES FANCY HEADER ---
...
\input{kapitel/Theorietische und technische Grundlagen.tex}
\clearpage

\input{kapitel/anforderungsanalyse.tex}
\clearpage

\input{kapitel/konzeptiondesprototyps.tex}
\clearpage
